...
static ssize_t logger_fake_message(int r_ver, char __user *buf, const char *fmt, ...) 
{
	...
	switch (r_ver) {
	case 1:
		header_size = sizeof(struct user_logger_entry_compat);
		break;
	case 2:
		header_size = sizeof(struct logger_entry);
		break;
	default:
		/* FIXME: ? */
		return 0;
...
start:
	...
	if (unlikely(s_fake_read)) {
		ret = logger_fake_message(reader->r_ver, buf, "Fake read return string......");
		goto out;
	}

	/* for android log missing warning { */
	if (reader->missing_bytes > 0) {
		ret = logger_fake_message(reader->r_ver, buf, "some logs have been lost (%u bytes estimated)", reader->missing_bytes);
