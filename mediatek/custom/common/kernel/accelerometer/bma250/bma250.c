...
#define BMA250_I2C_SLAVE_ADDR   0x18
#else
#define BMA250_I2C_SLAVE_ADDR   0x19
#endif
...
static struct i2c_board_info __initdata i2c_bma250={ I2C_BOARD_INFO(BMA250_DEV_NAME, BMA250_I2C_SLAVE_ADDR)};
...
/*                                                                                  */
static void BMA250_SetGPIO(void)
{
    //unset to default I2C_1
    mt_set_gpio_mode(222,0);
    mt_set_gpio_dir(222,0);
    mt_set_gpio_mode(224,0);
    mt_set_gpio_dir(224,0);

    //set to I2C_1 SCL,SDA
    mt_set_gpio_mode(110, 4);
    mt_set_gpio_mode(107, 4);

    //se to I2C_1 INT
    //mt_set_gpio_mode(70,1);

    GSE_LOG("GPIO[222] Mode:%d\nGPIO[224] Mode:%d\nGPIO[107] Mode:%d\n GPIO[110] Mode:%d\n"
            ,mt_get_gpio_mode(222),mt_get_gpio_mode(224)
            ,mt_get_gpio_mode(107),mt_get_gpio_mode(110));
...
static int bma250_set_bandwidth(struct i2c_client *client, unsigned char BW)
{
	...
	if (client == NULL) {
		...
		if (ARRAY_SIZE(bma250_valid_bw) > i) {
			comres = bma250_smbus_read_byte(client, BMA250_BANDWIDTH__REG, &data);
...
static ssize_t bma250_fast_calibration_x_show(struct device_driver *ddri, char *buf)
{
	struct i2c_client *client = bma250_i2c_client;
	struct bma250_i2c_data *bma250 = i2c_get_clientdata(client);
...
static ssize_t bma250_fast_calibration_x_store(struct device_driver *ddri, char *buf, size_t count)
{
	unsigned long data;
	signed char tmp;
...
//                                                 
	bma250_read_accel_xyz(bma250->client, &acc_cal_pre);
	mdelay(50);
//                                                 

	if (bma250_set_offset_target_x(bma250->client,
		(unsigned char)data) < 0)
	{
 		return -EINVAL;
	}

	if (bma250_set_cal_trigger(bma250->client, 1) < 0)
	{
 		return -EINVAL;
	}
...
//                                                 
	do {
		mdelay(2);
		bma250_get_cal_ready(bma250->client, &tmp);
//                                                 
		bma250_read_accel_xyz(bma250->client, &acc_cal);
...
static ssize_t bma250_fast_calibration_y_show(struct device_driver *ddri, char *buf)
{
...
static ssize_t bma250_fast_calibration_y_store(struct device_driver *ddri, char *buf, size_t count)
{
	unsigned long data;
	signed char tmp;
	unsigned int timeout = 0;
...
//                                                 
	...
	if (bma250_set_offset_target_y(bma250->client,
				(unsigned char)data) < 0)
		return -EINVAL;

	if (bma250_set_cal_trigger(bma250->client, 2) < 0)
		return -EINVAL;
...
static ssize_t bma250_fast_calibration_z_show(struct device_driver *ddri, char *buf)
{
...
static ssize_t bma250_fast_calibration_z_store(struct device_driver *ddri, char *buf, size_t count)
{
	unsigned long data;
	signed char tmp;
	unsigned int timeout = 0;
...
//                                                 
	...
	if (bma250_set_offset_target_z(bma250->client,
				(unsigned char)data) < 0)
		return -EINVAL;

	if (bma250_set_cal_trigger(bma250->client, 3) < 0)
		return -EINVAL;
...
static int bma250_set_ee_w(struct i2c_client *client, unsigned char eew)
{
	...
	comres = bma250_smbus_read_byte(client, BMA250_UNLOCK_EE_WRITE_SETTING__REG, &data);
	data = BMA250_SET_BITSLICE(data, BMA250_UNLOCK_EE_WRITE_SETTING, eew);
	comres = bma250_smbus_write_byte(client, BMA250_UNLOCK_EE_WRITE_SETTING__REG, &data);
	return comres;
}
...
static int bma250_set_ee_prog_trig(struct i2c_client *client)
{
	...
	comres = bma250_smbus_read_byte(client, BMA250_START_EE_WRITE_SETTING__REG, &data);
	data = BMA250_SET_BITSLICE(data, BMA250_START_EE_WRITE_SETTING, eeprog);
	comres = bma250_smbus_write_byte(client, BMA250_START_EE_WRITE_SETTING__REG, &data);
	return comres;
}
...
static ssize_t bma250_eeprom_writing_show(struct device_driver *ddri, char *buf)
{
	unsigned char data;
	struct i2c_client *client = bma250_i2c_client;
	struct bma250_i2c_data *bma250 = i2c_get_clientdata(client);

	if (bma250_get_eeprom_writing_status(bma250->client, &data) < 0)
		return sprintf(buf, "Read error\n");

	return sprintf(buf, "%d\n", data);
}

static ssize_t bma250_eeprom_writing_store(struct device_driver *ddri, char *buf, size_t count)
{
	unsigned long data;
	signed char tmp;
	int timeout = 0;
	...
	/* unlock eeprom */
	if (bma250_set_ee_w(bma250->client, 1) < 0)
		return -EINVAL;

	printk(KERN_INFO "unlock eeprom successful\n");

	if (bma250_set_ee_prog_trig(bma250->client) < 0)
		return -EINVAL;
	printk(KERN_INFO "start update eeprom\n");

	do {
		mdelay(2);
		bma250_get_eeprom_writing_status(bma250->client, &tmp);
	...
	/* unlock eeprom */
	if (bma250_set_ee_w(bma250->client, 0) < 0)
		return -EINVAL;
...
static ssize_t bma250_bandwidth_show(struct device_driver *ddri, char *buf)
{
	unsigned char data;
	struct i2c_client *client = bma250_i2c_client;
	struct bma250_i2c_data *bma250 = i2c_get_clientdata(client);
...
static ssize_t bma250_bandwidth_store(struct device_driver *ddri, char *buf, size_t count)
{
	unsigned long data;
	int error;
	struct i2c_client *client = bma250_i2c_client;
...
static ssize_t bma250_softreset_store(struct device_driver *ddri, char *buf, size_t count)
{
	struct i2c_client *client = bma250_i2c_client;
	struct bma250_i2c_data *bma250 = i2c_get_clientdata(client);

	if (bma250_soft_reset(bma250->client) < 0)
		return -EINVAL;
...
//                                                                              
static DRIVER_ATTR(fast_calibration_x, S_IRUGO|S_IWUSR|S_IWGRP, bma250_fast_calibration_x_show, bma250_fast_calibration_x_store);
static DRIVER_ATTR(fast_calibration_y, S_IRUGO|S_IWUSR|S_IWGRP, bma250_fast_calibration_y_show, bma250_fast_calibration_y_store);
static DRIVER_ATTR(fast_calibration_z, S_IRUGO|S_IWUSR|S_IWGRP, bma250_fast_calibration_z_show, bma250_fast_calibration_z_store);
static DRIVER_ATTR(eeprom_writing, S_IRUGO|S_IWUSR|S_IWGRP, bma250_eeprom_writing_show, bma250_eeprom_writing_store);
static DRIVER_ATTR(bandwidth, S_IRUGO|S_IWUSR|S_IWGRP, bma250_bandwidth_show, bma250_bandwidth_store);
//                                                 
static DRIVER_ATTR(softreset, S_IWUSR|S_IWGRP, NULL, bma250_softreset_store);
//                                                 
//                                                                              
...
{
	GSE_FUN();
	i2c_register_board_info(1, &i2c_bma250, 1);
