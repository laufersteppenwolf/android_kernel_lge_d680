...
#define BMA255_DOUBLE_TAP_CALIBRATION
...
#ifdef BMA255_DOUBLE_TAP_CALIBRATION
...
static unsigned char sign_value = 0;
		...
		char *intstatus)
{
	...
	comres = bma255_smbus_read_byte(client, BMA2X2_STATUS_ORIENT_HIGH_REG,
			&data);
	data = BMA255_GET_BITSLICE(data, BMA2X2_FLAT_S);
	*intstatus = data;
...
static int bma255_set_int1_pad_sel(struct i2c_client *client, unsigned char int1sel)
{
	...
	case PAD_SINGLE_TAP:
		...
		break;
	case PAD_FLAT:
		comres = bma255_smbus_read_byte(client,BMA2X2_EN_INT1_PAD_FLAT__REG, &data);
		data = BMA255_SET_BITSLICE(data, BMA2X2_EN_INT1_PAD_FLAT,state);
		comres = bma255_smbus_write_byte(client,BMA2X2_EN_INT1_PAD_FLAT__REG, &data);
		break;
	default:
		break;
	}
...
static int bma255_set_Int_Enable(struct i2c_client *client, unsigned char InterruptType , unsigned char value)
{
	...
	comres = bma255_smbus_read_byte(client, BMA255_INT_REG_1, &data1);
	comres = bma255_smbus_read_byte(client, BMA255_INT_REG_2, &data2);
	...
	case 11:
		/* Flat Interrupt */
		data1 = BMA255_SET_BITSLICE(data1, BMA2X2_EN_FLAT_INT, value);
		break;
	default:
		break;
	}
	
	comres = bma255_smbus_write_byte(client, BMA255_INT_REG_1, &data1);
	comres = bma255_smbus_write_byte(client, BMA255_INT_REG_2, &data2);
	...
	switch (status) {
		case 0x10:
			...
			if(sensor_suspend==true){
				bma255_smbus_read_byte(client,BMA2X2_STATUS_TAP_SLOPE_REG, &slope_data);
				printk("double tap slope_data = %d\n",slope_data);
				if(slope_data==192||slope_data==64){
					printk(KERN_INFO "double tap interrupt happened by Z\n");
...
//			if(sensor_suspend==true)kpd_send_pwrkey();
			break;
		case 0x80:
			bma2x2_get_orient_flat_status(client,&sign_value);
			printk(KERN_INFO "flat detection interrupt happened\n");
			if (sign_value == 1) {
				printk(KERN_INFO "flat = 1 detection interrupt happened %d\n",tap_threshold_flat);
				data = 0x01;
				bma255_smbus_write_byte(client, BMA2X2_TAP_THRES_REG, &tap_threshold_flat);
			} else {
				printk(KERN_INFO "flat = 0 detection interrupt happened %d\n",tap_threshold);
				data = 0x04;
				bma255_smbus_write_byte(client, BMA2X2_TAP_THRES_REG, &tap_threshold);
			}
			break;
		default:
			printk(KERN_INFO "Unknown interrupt: %x\n", status);
			break;
	}
...
static void bma255_setup_eint(void)
{
	...
	mt65xx_eint_set_sens(CUST_EINT_GSE_1_NUM, CUST_EINT_EDGE_SENSITIVE);
	mt65xx_eint_set_polarity(CUST_EINT_GSE_1_NUM, CUST_EINT_POLARITY_HIGH);
	mt65xx_eint_set_hw_debounce(CUST_EINT_GSE_1_NUM, CUST_EINT_GSE_1_DEBOUNCE_CN);
	mt65xx_eint_registration(CUST_EINT_GSE_1_NUM, CUST_EINT_GSE_1_DEBOUNCE_EN, CUST_EINT_POLARITY_HIGH, bma255_eint_func, 0);
		...
		duration)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_TAP_QUIET_DURN__REG,
			&data);
	data = BMA255_SET_BITSLICE(data, BMA2X2_TAP_QUIET_DURN, duration);
		...
		*status)
{
	...
	data = BMA255_GET_BITSLICE(data, BMA2X2_TAP_QUIET_DURN);
	*status = data;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (bma2x2_get_tap_quiet(bma2x2->client, &data) < 0)
		return sprintf(buf, "Read error\n");
		...
		const char *buf, size_t count)
{
	...
	if (bma2x2_set_tap_quiet(bma2x2->client, (unsigned char)data) <
			0)
		return -EINVAL;
		...
		threshold)
{
	...
	data = BMA255_SET_BITSLICE(data, BMA2X2_TAP_THRES, threshold);
		...
		*status)
{
	...
	data = BMA255_GET_BITSLICE(data, BMA2X2_TAP_THRES);
		...
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	struct bma255_i2c_data *bma2x2 =obj_i2c_data;
	if (bma2x2_get_tap_threshold(bma2x2->client, &data) < 0)
		return sprintf(buf, "Read error\n");
		...
		const char *buf, size_t count)
{
	...
	if (error)
		return error;
	if (bma2x2_set_tap_threshold(bma2x2->client, (unsigned char)data)
			< 0)
		return -EINVAL;

	return count;
}
static int bma2x2_set_tap_threshold_flat(struct i2c_client *client, unsigned char
		threshold)
{
	int comres = 0;
	unsigned char data;
...
static int bma2x2_get_tap_threshold_flat(struct i2c_client *client, unsigned char
		*status)
{
	int comres = 0;
	unsigned char data;
		...
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	struct bma255_i2c_data *bma2x2 =obj_i2c_data;
	if (bma2x2_get_tap_threshold_flat(bma2x2->client, &data) < 0)
		return sprintf(buf, "Read error\n");
		...
		const char *buf, size_t count)
{
	...
	if (error)
		return error;
	if (bma2x2_set_tap_threshold_flat(bma2x2->client, (unsigned char)data)
			< 0)
		return -EINVAL;
		...
		thetaflat)
{
	...
	data = BMA255_SET_BITSLICE(data, BMA2X2_THETA_FLAT, thetaflat);
		...
		*status)
{
	...
	data = BMA255_GET_BITSLICE(data, BMA2X2_THETA_FLAT);
	*status = data;
		...
		struct device_attribute *attr, char *buf)
{
	unsigned char data;
	struct bma255_i2c_data *bma2x2 =obj_i2c_data;
	if (bma2x2_get_theta_flat(bma2x2->client, &data) < 0)
		return sprintf(buf, "Read error\n");
		...
		const char *buf, size_t count)
{
	...
	if (bma2x2_set_theta_flat(bma2x2->client, (unsigned char)data) <
			0)
		return -EINVAL;
		...
		duration)
{
	...
	data = BMA255_SET_BITSLICE(data, BMA2X2_TAP_DUR, duration);
		...
		*status)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_TAP_PARAM_REG, &data);
	data = BMA255_GET_BITSLICE(data, BMA2X2_TAP_DUR);
	*status = data;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (bma2x2_get_tap_duration(bma2x2->client, &data) < 0)
		return sprintf(buf, "Read error\n");
		...
		const char *buf, size_t count)
{
	...
	if (bma2x2_set_tap_duration(bma2x2->client, (unsigned char)data)
			< 0)
		return -EINVAL;
...
static int bma2x2_set_tap_shock(struct i2c_client *client, unsigned char setval)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_TAP_SHOCK_DURN__REG,
			&data);
	data = BMA255_SET_BITSLICE(data, BMA2X2_TAP_SHOCK_DURN, setval);
		...
		*status)
{
	...
	comres = bma2x2_smbus_read_byte(client, BMA2X2_TAP_PARAM_REG, &data);
	data = BMA255_GET_BITSLICE(data, BMA2X2_TAP_SHOCK_DURN);
	*status = data;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (bma2x2_get_tap_shock(bma2x2->client, &data) < 0)
		return sprintf(buf, "Read error\n");
		...
		const char *buf, size_t count)
{
	...
	if (error)
		return error;
	if (bma2x2_set_tap_shock(bma2x2->client, (unsigned char)data) <
			0)
		return -EINVAL;
	return count;
}
#endif //BMA255_DOUBLE_TAP_CALIBRATION
...
//                                                            
#ifdef BMA255_DOUBLE_TAP_CALIBRATION
static DRIVER_ATTR(tap_threshold, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_tap_threshold_show, bma2x2_tap_threshold_store);
static DRIVER_ATTR(tap_threshold_flat, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_tap_threshold_flat_show, bma2x2_tap_threshold_flat_store);
static DRIVER_ATTR(tap_quiet, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_tap_quiet_show, bma2x2_tap_quiet_store);
static DRIVER_ATTR(tap_duration, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_tap_duration_show, bma2x2_tap_duration_store);
static DRIVER_ATTR(tap_shock, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_tap_shock_show, bma2x2_tap_shock_store);
static DRIVER_ATTR(tap_enable, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_tap_enable_show, bma2x2_tap_enable_store);
#endif
#if 1 //flat_theta
static DRIVER_ATTR(flat_theta, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_flat_theta_show, bma2x2_flat_theta_store);
#endif
/*----------------------------------------------------------------------------*/
...
static int bma255_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	bma255_smbus_write_byte(client, BMA2X2_TAP_PARAM_REG, &data);
	data = 0x01;
	bma255_smbus_write_byte(client, BMA2X2_TAP_THRES_REG, &data);
	data = 0x05; //20 degree
	bma255_smbus_write_byte(client, BMA2X2_THETA_FLAT__REG, &data);
#endif
