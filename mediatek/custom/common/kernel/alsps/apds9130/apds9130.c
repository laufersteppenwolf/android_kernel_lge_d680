...
static void apds9130_setup_eint ( void )
{
	...
	mt65xx_eint_registration ( CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_EN, CUST_EINT_ALS_POLARITY, apds9130_eint_func, 0 );
...
static long apds9130_initialize ( struct i2c_client *client  )
{
	...
	apds9130_set_ppcount ( client, 0x10 ); /* 10-Pulse for proximity */
	apds9130_set_control ( client, 0x64 ); /* 50mA, IR-diode, 2X PGAIN */

	/* crosstalk value shall be set by LGP Server using I/O so init here to 700 */
    obj->ps_cross_talk = 700;
	...
	obj->ps_th_far_low = 0;
	obj->ps_th_far_high = 150 + obj->ps_cross_talk;
