...
static void apds9190_led_power ( unsigned int on )
{
	...
	if ( led_power_on != on )
	{
		if ( on )
		{
			if ( !hwPowerOn ( MT65XX_POWER_LDO_VCAMA, VOL_2800, "APDS9190LEDA" ) )
			{
		...
		{
			if ( !hwPowerDown ( MT65XX_POWER_LDO_VCAMA, "APDS9190LEDA" ) )
			{
...
static struct platform_driver apds9190_alsps_driver = { .probe = apds9190_probe, .remove = apds9190_remove, .driver = { .name = "als_ps", } };
