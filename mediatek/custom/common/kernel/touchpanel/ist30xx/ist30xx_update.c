...
#include "ist30xx_fw_single.h"
#else
#include "ist30xx_fw_dual.h"
#endif
#endif
...
int ist30xx_calib_wait(void)
{
	...
	while (cnt-- > 0) {
		...
		if (ts_data->status.calib_msg) {
			TPD_LOG("Calibration Status : %d, Max raw gap : %d - (raw: %08x)\n",
...
int ist30xx_fw_param_update(struct i2c_client *client, const u8 *buf)
{
	...
	TPD_LOG("*** F/W update ***\n");
	TPD_LOG("Core ver: %08x, addr: 0x%04x ~ 0x%04x\n", ist30xx_fw_ver, ts_data->fw.index, ts_data->fw.index + ts_data->fw.size);
...
int ist30xx_check_fw(struct ist30xx_data *data, const u8 *buf)
{
	...
	chksum_bin = ist30xx_make_checksum(CHKSUM_ALL, buf);

	ret = ist30xx_read_cmd(data->client, CMD_GET_CHECKSUM_ALL, &chksum);
	if (ret) return ret;

	if (chksum != chksum_bin) {
		printk("[ TSP ] Checksum compare error %08x, %08x\n", chksum, chksum_bin);
		return -EPERM;
	}
...
//                                
#define V4_MODEL(k)     ((k >> 8) & 0xFF)
#define V4_MODEL_DUAL   (1)
#define V4_MODEL_SINGLE (2)
int ist30xx_check_auto_update(struct ist30xx_data *data)
{
	ist30xx_write_cmd(data->client, CMD_RUN_DEVICE, 0);
	msleep(10);

	if (ist30xx_check_fw(data, ist30xx_fw))
		return -EPERM;
	...
	printk("[ TSP ] bin fw: %x, param: %x\n", ist30xx_fw_ver, ist30xx_param_ver);

	if ((data->fw.ver & MASK_FW_VER) != IST30XX_FW_VER3) // FW core over 3.0
		return -EPERM;

	// Check V4 dual / single
	if (V4_MODEL(data->param_ver) == V4_MODEL(ist30xx_param_ver))
	{
		if ((data->fw.ver >= ist30xx_fw_ver) && (data->param_ver >= ist30xx_param_ver))
		{
			TPD_LOG("[ TSP ] Touch F/W version is updated already!\n");
			return 0;
		}
		else
		{
			return -EAGAIN; // FW update
		}
	}
	else if (V4_MODEL(data->param_ver) == V4_MODEL_DUAL || V4_MODEL(data->param_ver) == V4_MODEL_SINGLE)
	{
		printk("[ TSP ] Image F/W version is incorrect! (Touch: 0x%04x, Image: 0x%04x)\n", data->param_ver, ist30xx_param_ver);
		return 0; // TSP type is incorrect (Dual,Triple / Single)
	}

	printk("[ TSP ] Touch IC F/W version is wrong! (0x%04x)\n", data->param_ver);

	return -EPERM; // FW recovery
}
...
update_bin:   // TSP is not ready / FW update
	printk("[ TSP ] Update version FW: %08x -> %08x, Param: %04x -> %04x\n",
	...
	if (retry > 0 && ist30xx_check_fw(data, ist30xx_fw))
		goto update_bin;
...
ssize_t ist30xx_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;

	count = sprintf(buf, "Core ver: 0x%x, F/W ver: 0x%x\n", ts_data->fw.ver, ts_data->param_ver);
...
/* sysfs  */
static DEVICE_ATTR(firmware, 0664, ist30xx_fw_status, ist30xx_fw_store);
static DEVICE_ATTR(fwparam, 0664, NULL, ist30xx_fwparam_store);
static DEVICE_ATTR(version, 0664, ist30xx_fw_version, NULL);
static DEVICE_ATTR(param, 0664, NULL, ist30xx_param_store);
