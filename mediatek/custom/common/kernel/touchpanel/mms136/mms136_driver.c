...
//single
#define MELFAS_CURRENT_FW_VERSION 0x00
#define MELFAS_CURRENT_FW_VERSION_R50 0x00
#else
//dual, triple
#if defined(CONFIG_TOUCH_IC_ID)
#define MELFAS_CURRENT_FW_VERSION 0x02
#define MELFAS_CURRENT_FW_VERSION_R50 0x02
#else
#define MELFAS_CURRENT_FW_VERSION 0x00
#define MELFAS_CURRENT_FW_VERSION_R50 0x00
#endif
...
static int tpd_keys_local[TPD_KEY_COUNT] = {KEY_BACK ,KEY_HOME, KEY_MENU};
#endif
/*                                                              */
...
static ssize_t store_update_pro(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (hw_ver == 0) {
		melfas_ts_fw_load(melfas_i2c_client, 0);
	} else {
		melfas_ts_fw_load(melfas_i2c_client, 1);
	}
...
#if 1
static DEVICE_ATTR(update, 0644, show_update_pro, store_update_pro);
#else
...
static int melfas_firmware_update(struct i2c_client *client)
{
    ...
    {
#if 0
    	if (fw_ver[0] == 0x02) { // 150 Ohm
			if (fw_ver[1] < MELFAS_CURRENT_FW_VERSION)
				ret = melfas_ts_fw_load(client, 0);
    	} else { // 270 Ohm
...
static ssize_t melfas_FW_show(struct device *dev,  struct device_attribute *attr,  char *buf)
{
    ...
    i2c_smbus_read_i2c_block_data(melfas_i2c_client, TS_READ_HW_VER_ADDR, sizeof(product_id2), &product_id2);

	return sprintf(buf, "%u : FW_VERSION, 0x%02x : HW_VERSION \n", product_id, product_id2);
}

static DEVICE_ATTR(fw, 0644, melfas_FW_show, NULL);
...
{             
	...
	#if 1 // to avoid hang up after booted up
	tpd_hw_disable();
	tpd_hw_enable();
    #endif	
...
static int __init melfas_tpd_driver_init(void)
{
    ...
    printk("\n\n[ TPD ] melfas_tpd_driver_init!\n"); 
    gpio_status = mt_get_gpio_in(GPIO_TOUCH_IC_ID);
    printk("\n\n[ TPD ] TOUCH_IC_ID[%d] \n",gpio_status);

    if(mt_get_gpio_in(GPIO_TOUCH_IC_ID))
    {
        printk("[ TPD ] Imagis driver  \n");
        return 0;
    }
    else
    {
        printk("[ TPD ] melfas IC driver \n");
    }	
#endif /*                                                                   */
