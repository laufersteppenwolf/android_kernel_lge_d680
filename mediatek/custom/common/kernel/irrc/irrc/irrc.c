...
static long IRRC_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IRRC_START:
		...
		if(system_rev<2){
			mt_set_gpio_mode(GPIO_RC_LED_LDO_EN,GPIO_RC_LED_LDO_EN_M_GPIO);
			mt_set_gpio_pull_enable(GPIO_RC_LED_LDO_EN, GPIO_PULL_ENABLE);
			mt_set_gpio_dir(GPIO_RC_LED_LDO_EN,GPIO_DIR_OUT);
			mt_set_gpio_out(GPIO_RC_LED_LDO_EN,GPIO_OUT_ONE);
		}else if(system_rev==2){
			hwPowerOn(MT65XX_POWER_LDO_VCAMA, VOL_2800, "2V8_IrRC_LDO_S");
		}else{
			mt_set_gpio_mode(GPIO_IRRC_LDO_EN,GPIO_IRRC_LDO_EN_M_GPIO);
