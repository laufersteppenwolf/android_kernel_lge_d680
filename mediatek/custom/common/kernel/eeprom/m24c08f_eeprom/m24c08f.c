...
static struct i2c_board_info __initdata kd_eeprom_dev={ I2C_BOARD_INFO("EEPROM_M24C08F", 0xAA>>1)};
...
*
********************************************************************************/
#define EEPROM_DRVNAME "EEPROM_M24C08F"
...
#define EEPROM_Address_Offset 0xA
...
//int iReadDataTest(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
int iReadDataFromM24C08F(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{

		char puSendCmd[1] = {(char)(ui4_offset & 0xFF) };
		unsigned char SampleOffset = (char)(ui4_offset & 0xFF) ;
        unsigned char EEPROM_Address[2] = {0x50,0x50} ;
        ...
        for(loopCount=0; loopCount < Read_NUMofEEPROM; loopCount++)
        {
               do 
               {
                   if( 16 <= loop[loopCount])
                   {
                       ...
                       //EEPROMDB("[EEPROM]1 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x \n",loopCount ,loop[loopCount],puSendCmd[0] );
                       iReadRegI2C(puSendCmd , 1, (u8*)pBuff,16,EEPROM_Address[loopCount]);
                       if (i4RetValue != 0)
                       {
                   ...
                   {
                       //EEPROMDB("[EEPROM]2 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x \n",loopCount ,loop[loopCount],puSendCmd[0] );
                       iReadRegI2C(puSendCmd , 1, (u8*)pBuff,loop[loopCount],EEPROM_Address[loopCount]);
                       if (i4RetValue != 0)
                       {
                            EEPROMDB("[EEPROM] I2C iReadData failed!! \n");
                            return -1;
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case EEPROMIOC_G_READ:
            ...
            if(ptempbuf->u4Offset == 0xFFFF)
            {
              int loop;
              *(u16 *)pWorkingBuff = 0xF50;
              
            }
            else if(ptempbuf->u4Offset == 0x7E7F)
            {
                char puSendCmd[1] = {0, };
...
//    spin_lock_init(&g_EEPROMLock);
    ...
    g_pstI2Cclient->addr = M24C08F_DEVICE_ID>>1;
    spin_unlock(&g_EEPROMLock); // for SMP    
