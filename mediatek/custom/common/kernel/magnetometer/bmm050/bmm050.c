...
#endif
				data_count++;
				if(data_count%500==0)MSE_LOG("Hwm get m-sensor data: %d, %d, %d. divide %d, status %d!\n", msensor_data->values[0],msensor_data->values[1],msensor_data->values[2],
						msensor_data->value_divide,msensor_data->status);
			}
			break;
		default:
...
/*----------------------------------------------------------------------------*/
static void bmm050_shutdown(struct i2c_client *client)
{
	MSE_ERR("############## BMA255 SHUTDOWN###############\n");
	if (!hwPowerDown(MT65XX_POWER_LDO_VCAM_AF, "BMM050"))
	{
		MSE_ERR("power off fail!!\n");
	}
}
