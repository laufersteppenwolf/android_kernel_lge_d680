...
#else
    mt_set_gpio_mode(GPIO192, GPIO_MODE_03);    /* set to mode3: CLKM4 */
    mt_set_gpio_dir(GPIO192, GPIO_DIR_OUT);     /* set to output */
    /* dev : 1, source clock and output clock is same */
    mt_set_clock_output(CLK_OUT4, CLK_SRC_F32K, 1); /* set clock & clock source for CLKM4 */
#endif
/*                                                                                                  */
