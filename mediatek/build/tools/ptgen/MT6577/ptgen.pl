...
#Generate config.txt & file_list.txt files
my $boot1_size = 0;  # boot1 partition size of eMMC
my $boot2_size = 0;  # boot2 partition size of eMMC
my $rpmb_size = 0;   # RPMB partition size of eMMC
my $storage_size = 0; # storage size of eMMC
...
if($ARGV[11]=~/LGE_PARTITION_TABLE_TYPE=(.*)/){
	$LGE_PARTITION_TABLE_TYPE = $1;
}
my $PART_TABLE_FILENAME   = "mediatek/build/tools/ptgen/$PLATFORM/partition_table_${PLATFORM}.xls"; # excel file name
if ($LGE_PARTITION_TABLE_TYPE ne "") {
	$PART_TABLE_FILENAME   = "mediatek/build/tools/ptgen/$PLATFORM/partition_table_${LGE_PARTITION_TABLE_TYPE}.xls"; # specific excel file name
}

die "Cannot find ${PART_TABLE_FILENAME}!" if (! -e $PART_TABLE_FILENAME);
...
# LGE_UPDATE_S] for LGE FOTA dms_ua@lge.com 2012.09.13
my $LGFOTA_PARTITION_H = "vendor/lge/system/fota/oem/device/lgfota_mtk_partition.h";
# LGE_UPDATE_E] for LGE FOTA dms_ua@lge.com 2012.09.13

#Generate config.txt & file_list.txt files
my $OUT_CONFIG_DIR = "out/target/product/$PROJECT";
my $OUT_CONFIG_TXT = "out/target/product/$PROJECT/config.txt";
my $OUT_FILE_LIST_TXT = "out/target/product/$PROJECT/file_list.txt";
...
#			print "$'\n";
	...
	foreach $part_num (@lines) {
		if(exists $REGION_TABLE{$part_num}){
			...
			#Generate config.txt & file_list.txt files
			$cur_b1 = $REGION_TABLE{$part_num}{BOOT1}; #sprintf("0x%x",$REGION_TABLE{$part_num}{BOOT1}*1024);
			$cur_b2 = $REGION_TABLE{$part_num}{BOOT2}; #sprintf("0x%x",$REGION_TABLE{$part_num}{BOOT2}*1024);
			$cur_rpmb = $REGION_TABLE{$part_num}{RPMB}; #sprintf("0x%x",$REGION_TABLE{$part_num}{RPMB}*1024);

			if($cur_b1 > $boot1_size || $boot1_size == 0) {
				$boot1_size = $cur_b1;
			}
			if($cur_b2 > $boot2_size || $boot2_size == 0) {
				$boot2_size = $cur_b2;
			}
			if($cur_rpmb > $rpmb_size || $rpmb_size == 0) {
				$rpmb_size = $cur_rpmb;
			}
...
#		$combo_start_address = 6*1024; #Fix Me!!!
	...
	#Generate config.txt & file_list.txt files
	$storage_size = $MBR_Start_Address_KB + $Min_user_region;
	&GenConfigFile ();
}
...
#convert dec start_address to hex start_address
	...
	#Generate config.txt & file_list.txt files
	if (-e $OUT_FILE_LIST_TXT) {
		`chmod 777 $OUT_FILE_LIST_TXT`;
	}
	open (OUT_FILE_LIST_TXT, ">$OUT_FILE_LIST_TXT") or &error_handler("Ptgen open $OUT_FILE_LIST_TXT Fail!", __FILE__, __LINE__) ;
	$filelist .= "$total_rows\n";
	my $max_leng;
	
	$max_leng = length($PROJECT) + length("preloader_.bin") + 2;
	
	for($row=0;$row < $total_rows;$row++){
		if($DL_FIELD[$row] eq "0") {
			$filelist_temp = "__NODL_$PARTITION_FIELD[$row]";
			$filelist .= sprintf("%-*s", $max_leng, $filelist_temp);
		} elsif ($PARTITION_FIELD[$row] eq "PRELOADER") {
			$filelist .= sprintf("%-*s", $max_leng, "preloader_$PROJECT.bin");
		} elsif ($TYPE_FIELD[$row] eq "EXT4") {
			if ($PARTITION_FIELD[$row] eq "ANDROID") {
				$filelist .= sprintf("%-*s", $max_leng, "system.img");
			} elsif ($PARTITION_FIELD[$row] eq "SEC_RO") {
				$filelist .= sprintf("%-*s", $max_leng, "secro.img");
			} elsif ($PARTITION_FIELD[$row] eq "USRDATA") {
				$filelist .= sprintf("%-*s", $max_leng, "userdata.img");
			} else {
				$filelist_temp = lc($PARTITION_FIELD[$row]);
				$filelist_temp .= ".img";
				$filelist .= sprintf("%-*s", $max_leng, $filelist_temp);
			}
		} elsif ($PARTITION_FIELD[$row] eq "UBOOT") {
			$filelist .= sprintf("%-*s", $max_leng, "lk.bin");
		} elsif ($PARTITION_FIELD[$row] eq "BOOTIMG") {
			$filelist .= sprintf("%-*s", $max_leng, "boot.img");
		} elsif ($PARTITION_FIELD[$row] eq "RECOVERY") {
			$filelist .= sprintf("%-*s", $max_leng, "recovery.img");
		} elsif ($PARTITION_FIELD[$row] eq "LOGO") {
			$filelist .= sprintf("%-*s", $max_leng, "logo.bin");
		} else {
			$filelist .= sprintf("%-*s", $max_leng, $PARTITION_FIELD[$row]);
		}
		
		
		if($DL_FIELD[$row] eq "0") {
			$filelist .= sprintf("__NODL_%-15s", $PARTITION_FIELD[$row]);
		} else {
			$filelist .= sprintf("%-22s", $PARTITION_FIELD[$row]);
		}

		$filelist .= sprintf("0x%-10s", $START_FIELD_Byte_HEX[$row]);
	
		$filelist .= sprintf("0x%-3s", "1");
	
		if ($DL_FIELD[$row] eq "0") {
			if($PARTITION_FIELD[$row] eq "BMTPOOL") {
				$filelist .= sprintf("%-8s", "BMT");
			} elsif ($PARTITION_FIELD[$row] eq "PMT") {
				$filelist .= sprintf("%-8s", "PMT");
			} else {
				$filelist .= sprintf("%-8s", "__NODL");
			}
		} else {
			if($TYPE_FIELD[$row] eq "EXT4") {
				$filelist .= sprintf("%-8s", "EXT");
			} else {
				$filelist .= sprintf("%-8s", "RAW");
			}
		}
	
		if($REGION_FIELD[$row] eq "BOOT_1") {
			$filelist .= sprintf("%-3s", "1");
		} else {
			$filelist .= sprintf("%-3s", "0");
		}
		$filelist .= "\n";
	}    
	print OUT_FILE_LIST_TXT $filelist;
	close OUT_FILE_LIST_TXT ;
...
#****************************************************************************
# subroutine:  GenConfigFile - Generate config.txt & file_list.txt files
# return:      
#****************************************************************************
sub GenConfigFile ()
{
	...
	if (-e $OUT_CONFIG_TXT)	{
		`chmod 777 $OUT_CONFIG_TXT`;
	}
	open (OUT_CONFIG_TXT, ">$OUT_CONFIG_TXT") or &error_handler("Ptgen open $OUT_CONFIG_TXT Fail!", __FILE__, __LINE__) ;
	
	$boot1_size = sprintf("0x%x",$boot1_size*1024);
	$boot2_size = sprintf("0x%x",$boot2_size*1024);
	$rpmb_size = sprintf("0x%x",$rpmb_size*1024);
	$storage_size = sprintf("0x%x",$storage_size*1024);
	$temp .= "nfi_block_size_in_byte       0x80000\n";	
	$temp .= "nfi_page_size_in_byte        0x200\n";	
	$temp .= "boot1_patition_size_in_byte  $boot1_size\n";	
	$temp .= "boot2_patition_size_in_byte  $boot2_size\n";	
	$temp .= "rpmb_size_in_byte            $rpmb_size\n";	
	$temp .= "mtk_platform_flash_tool      $PLATFORM\n";	
	$temp .= "mtk_storage_flash_tool       ";
	if($EMMC_SUPPORT eq "yes") {
		$temp .= "EMMC\n";
	} else {
		$temp .= "NAND\n";
	}
	$temp .= "mtk_storage_size_in_byte     $storage_size";
		
	print OUT_CONFIG_TXT $temp;
	close OUT_CONFIG_TXT ;
}
...
# Fill MBR & EBR table -----------------------------------------------------
	...
	$BR[0][1][0][1] = $p1_start_block = 0x400;
	...
	$BR[1][1][1][1] = $p6_start_block - 0x400 - 0x400;
	$BR[1][1][1][2] = $p6_size_block + 0x400;
...
# LGE_UPDATE_S] for LGE FOTA dms_ua@lge.com 2012.09.13
#****************************************************************************************
# subroutine:  LGEFOTA_GenPartitionH
# return:
#****************************************************************************************
sub LGEFOTA_GenPartitionH
{
    my $iter = 0 ;
    my $temp ;

	if (-e $LGFOTA_PARTITION_H)
	{
		`chmod 777 $LGFOTA_PARTITION_H`;
	}
    open (LGFOTA_PARTITION_H, ">$LGFOTA_PARTITION_H") or &error_handler("Ptgen open LGFOTA_PARTITION_H fail!\n", __FILE__, __LINE__);

    print LGFOTA_PARTITION_H "\n#ifndef __LGE_FOTA_PARTITION_DEFINE_H__\n#define __LGE_FOTA_PARTITION_DEFINE_H__\n\n" ;
    print LGFOTA_PARTITION_H "\n\n\n#define KB_LGFOTA  (1024)\n#define MB_LGFOTA  (1024 * KB_LGFOTA)\n#define GB_LGFOTA  (1024 * MB_LGFOTA)\n\n" ;
...
#size
    for ($iter=0; $iter< $total_rows; $iter++)
    {
        ...
        {
			my $bmtpool=sprintf("%x",$SIZE_FIELD_KB[$iter]/64/$Page_Size);
			$temp = "#define LGFOTA_PART_SIZE_$PARTITION_FIELD[$iter]\t\t\t(0x$bmtpool)\n" ;
    		print LGFOTA_PARTITION_H $temp ;
        }else
        {
    		$temp = "#define LGFOTA_PART_SIZE_$PARTITION_FIELD[$iter]\t\t\t($SIZE_FIELD_KB[$iter]*KB_LGFOTA)\n" ;
			print LGFOTA_PARTITION_H $temp ;
        }
        
    }

    print LGFOTA_PARTITION_H "\n\n#define LGFOTA_PART_NUM\t\t\t$total_rows\n\n";
    print LGFOTA_PARTITION_H "\n\n#define LGFOTA_PART_MAX_COUNT\t\t\t 40\n\n";
	print LGFOTA_PARTITION_H "#define LGFOTA_MBR_START_ADDRESS_BYTE\t\t\t($MBR_Start_Address_KB*KB_LGFOTA)\n\n";
	if($EMMC_SUPPORT eq "yes"){
		print LGFOTA_PARTITION_H "#define LGFOTA_WRITE_SIZE_Byte		512\n";
	}else{
		print LGFOTA_PARTITION_H "#define LGFOTA_WRITE_SIZE_Byte		($Page_Size*1024)\n";
	}
