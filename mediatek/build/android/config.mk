...
BOARD_CONFIG_DIR ?= $(MTK_ROOT_CONFIG)/$(TARGET_PRODUCT)/
ARTIFACT_MODULE :=
ARTIFACT_TARGET := 
...
ifeq ($(HW_REVA),TRUE)
COMMON_GLOBAL_CFLAGS += -DLGE_HW_REVA
COMMON_GLOBAL_CPPFLAGS += -DLGE_HW_REVA
else
  ifeq ($(HW_REVB),TRUE)
  COMMON_GLOBAL_CFLAGS += -DLGE_HW_REVB
  COMMON_GLOBAL_CPPFLAGS += -DLGE_HW_REVB
  else
    ifeq ($(HW_REVC),TRUE)
    COMMON_GLOBAL_CFLAGS += -DLGE_HW_REVC
    COMMON_GLOBAL_CPPFLAGS += -DLGE_HW_REVC
    else
    COMMON_GLOBAL_CFLAGS += -DLGE_HW_REV1
    COMMON_GLOBAL_CPPFLAGS += -DLGE_HW_REV1
    endif
  endif
endif
