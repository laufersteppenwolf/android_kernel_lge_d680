...
include mediatek/build/makelghw.mk
$(call codebase-path)
...
REMAKECMD    :=  make -fmediatek/build/emakemtk.mk CMD_ARGU=$(CMD_ARGU) $(CMD_ARGU2) $(MAKE_DEBUG)
REMAKECMD_E    :=  emake -fmediatek/build/emakemtk.mk CMD_ARGU=$(CMD_ARGU) $(CMD_ARGU2) $(MAKE_DEBUG)
...
ANDROID_NATIVE_TARGETS := \
         ...
         persist_lgimage persistimage mptimage lgfotaimage swapimage
	...
	$(hide) \
        $(if $(filter update-api,$@),\
          $(if $(filter true,$(strip $(BUILD_TINY_ANDROID))), \
            ...
            $(REMAKECMD_E) ACTION=$@ CUR_MODULE=$@ android \
           ) \
          , \
          $(if $(filter snod userdataimage-nodeps,$@), \
            , \
          ...
          $(if $(filter banyan_addon,$@), \
            $(REMAKECMD_E) ACTION=sdk_addon CUR_MODULE=sdk_addon android \
            , \
            $(REMAKECMD_E) ACTION=$@ CUR_MODULE=$@ android \
           ) \
         )
...
mm:
	...
	$(hide) (source build/envsetup.sh;cd $(MM_PATH);TARGET_PRODUCT=$(TARGET_PRODUCT) FLAVOR=$(FLAVOR) org_mm $(SNOD) $(DEAL_STDOUT_MM);exit $${PIPESTATUS[0]})  && \
          $(SHOWRSLT) $$? $(LOG)$@.log || \
          $(SHOWRSLT) $$? $(LOG)$@.log
...
ifneq ($(PROJECT),generic)
	$(hide) echo $(SHOWTIME) $@ing...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
# LGE_CHANGE_S 2012-11-01 johny.kim@lge.com : add $(LGE_DWS) to seperate dws file from featuring
	cp mediatek/custom/$(PROJECT)/kernel/dct/lge/codegen$(LGE_DWS).dws mediatek/custom/$(PROJECT)/kernel/dct/$(PRIVATE_CUSTOM_KERNEL_DCT)/codegen.dws
	$(hide) mediatek/dct/DrvGen mediatek/custom/$(PROJECT)/kernel/dct/$(PRIVATE_CUSTOM_KERNEL_DCT)/codegen.dws $(DEAL_STDOUT_DRVGEN) && \
	 $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log || \
	 $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log
# LGE_CHANGE_E 2012-11-01 johny.kim@lge.com : add $(LGE_DWS) to seperate dws file from featuring
endif
...
ifneq ($(PROJECT), generic)
	...
	$(hide) perl $(MEM_PARTITION_GENERATOR) \
                     ...
                     MTK_HYBRID_PRODUCTFILE=$(MTK_HYBRID_PRODUCTFILE) \
                     $(DEAL_STDOUT_PTGEN) && \
                     $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log || \
                     $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log
...
ifneq (,$(filter MT6575 MT6577 MT6573,$(MTK_PLATFORM)))
	...
	$(hide) cd $(UBOOT_WD) && \
	  (MAKEJOBS=$(MAKEJOBS) ./ebuild.sh $(ACTION) $(DEAL_STDOUT) && \
	  cd $(MKTOPDIR) && \
...
ifeq ($(BUILD_KERNEL),yes)
  ...
  ifneq ($(KMOD_PATH),)
	...
	$(hide) cd $(KERNEL_WD) && \
	(KMOD_PATH=$(KMOD_PATH) ./ebuild.sh $(ACTION) $(KERNEL_ARG) ) && cd $(MKTOPDIR)
  else
	$(hide) echo $(SHOWTIME) $(SHOWBUILD)ing $@...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_MODULE_LOG)
	$(hide) cd $(KERNEL_WD) && \
	  (MAKEJOBS=$(MAKEJOBS) ./ebuild.sh $(ACTION) $(PROJECT) $(DEAL_STDOUT) && \
	   cd $(MKTOPDIR) && \
...
ifeq ($(SHOWBUILD), build)
	...
	$(hide) $(EMAKECMD) $(ACTION) $(DEAL_STDOUT) && \
