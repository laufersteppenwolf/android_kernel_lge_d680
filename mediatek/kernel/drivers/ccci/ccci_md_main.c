...
void ccci_sys_rx_cb(CCCI_BUFF_T *buff, void *private_data);
...
static int boot_md(void)
{
	...
	ret = ccci_register(CCCI_SYSTEM_RX, ccci_sys_rx_cb, NULL);
	if (ret != 0 && ret != CCCI_IN_USE) {
		CCCI_MSG_INF("ctl", "fail to register CCCI_SYSTEM_RX\n");
		ccci_unregister(CCCI_SYSTEM_RX);
		return ret;
	}
...
void ccci_sys_rx_cb(CCCI_BUFF_T *buff, void *private_data)
{
	//CCCI_MSG_INF("cci", "ccci_sys_rx_cb \n");
	CCCI_BUFF_T sys_msg;
	
	if(buff->data[1] == MD_SLP_REQUEST) {
		if (buff->reserved == LOCK_MD_SLP) {
			md_slp_lock_ack++;
			//CCCI_MSG_INF("cci", "ack md lock sleep OK! \n");
		}
		else if (buff->reserved == UNLOCK_MD_SLP) {
			md_slp_unlock_ack++;
			//CCCI_MSG_INF("cci", "ack md unlock sleep OK! \n");
		}
		else if (buff->reserved == 0xFFFF) {
			CCCI_MSG_INF("cci", "request md sleep fail! \n");
		}
	} else if (buff->data[1] == MD_WDT_MONITOR) {
		CCCI_MSG_INF("cci", "MD WDT monitor message, start timer\n");
		//start_md_wdt_recov_timer();
	} else if (buff->data[1] == MD_GET_BATTERY_INFO) {
		CCCI_INIT_MAILBOX(&sys_msg, CCCI_MD_MSG_SEND_BATTERY_INFO);
    	        ccci_system_message(&sys_msg);
		CCCI_MSG_INF("cci", "request to send battery voltage to md\n");		
	}
...
void send_battery_info(int data)
{
	int val = 0;
	int ret = 0;
	unsigned int para = 0;
	
	val = get_bat_info(para);
	ret = ccci_write_mailbox_with_resv(CCCI_SYSTEM_TX, MD_GET_BATTERY_INFO, val);
	CCCI_MSG_INF("cci", "send battery vol(%d) to md: %d\n", val, ret);	
}
