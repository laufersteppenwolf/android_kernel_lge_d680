...
extern void send_battery_info(int data);
...
static int ccci_dev_open(struct inode *inode, struct file *file)
{
	...
	if (index == CCCI_SYSTEM_RX) {
		CCCI_MSG_INF("chr", "Open ch%d by %s not permit\n", index, current->comm);
		return -1;
	}
	
	client = find_get_client(index, current->pid);
	CCCI_MSG_INF("chr", "Open ch%d by %s\n", index, current->comm);
	if (IS_ERR(client))
	{
...
static int ccci_dev_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    else if (client->ch_num == CCCI_UEM_RX) {
	get_md_mem_info(&addr,&len);	
    }
