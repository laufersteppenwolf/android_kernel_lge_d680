...
DISP_STATUS DISP_PanelEnable(BOOL enable)
{
	...
	else if (!enable && s_enabled)
	{
		...
		DISP_LOG("LCD Suspend start");
		lcm_drv->suspend();
		DISP_LOG("LCD Suspend end");
	}
...
/*                                                                          */
extern void DSI_wait_frame_end(void);
/*                                                                          */
...
{
	lcm_drv->update(0, 0, 320, 480);
    LCD_CHECK_RET(LCD_SetRoiWindow(0, 0, 320, 480));
