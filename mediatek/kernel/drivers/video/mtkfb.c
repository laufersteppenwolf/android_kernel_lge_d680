...
struct fb_overlay_layer video_layer_context[VIDEO_LAYER_COUNT];
...
void mtkfb_pan_disp_test(void)
{
	...
	if (down_interruptible(&sem_flipping)) {
        MTKFB_LOG("[fb driver] can't get semaphore:%d\n", __LINE__);
        return;
    }
	sem_flipping_cnt--;
...
{
	MTKFB_FUNC();
	MTKFB_LOG("mtkfb_set_backlight_level:%d\n", level);
	if (down_interruptible(&sem_flipping)) {
        MTKFB_LOG("[FB Driver] can't get semaphore:%d\n", __LINE__);
        return -ERESTARTSYS;
    }
    ...
    if (is_early_suspended){
		...
		MTKFB_LOG("[FB driver] set backlight level but FB has been suspended\n");
		goto End;
    	}
...
void mtkfb_switch_factory_to_normal(void)
{
    ...
    if (down_interruptible(&sem_flipping)) {
        MTKFB_LOG("[FB Driver] can't get semaphore in mtkfb_switch_factory_to_normal()\n");
        return;
    }
	sem_flipping_cnt--;
	if (down_interruptible(&sem_early_suspend)) {
...
//		if(!is_early_suspended) return r;
    ...
    case MTKFB_CAPTURE_FRAMEBUFFER:
    {
        ...
        {
		    unsigned int disabled_for_protect, layer;

            disabled_for_protect = 0;
			for (layer = LCD_LAYER_0; layer < VIDEO_LAYER_COUNT; layer++) {

				// check enabled layers need to be disabled for protection
                if ((true == LCD_IsLayerEnable(layer)) && (1 == video_layer_context[layer].protect)) {
					disabled_for_protect |= (0x80000000 >> layer);
                }
			}

            // content protection required
            if (0 != disabled_for_protect) {
    			for (layer = LCD_LAYER_0; layer < VIDEO_LAYER_COUNT; layer++) {

    				// disable protected layer.
	    			if (0 != (disabled_for_protect & (0x80000000 >> layer))) {
            ...
            // set back after content protection
            if (0 != disabled_for_protect) {
	    		for (layer = LCD_LAYER_0; layer < VIDEO_LAYER_COUNT; layer++) {
    ...
    case MTKFB_SET_VIDEO_LAYERS:
    {
        ...
        if (copy_from_user(&video_layer_context, (void __user *)arg, sizeof(video_layer_context))) {
            MTKFB_LOG("[FB]: copy_from_user failed! line:%d \n", __LINE__);
            r = -EFAULT;
        } else {
            int32_t i;
...
*/
            ...
            for (i = 0; i < VIDEO_LAYER_COUNT; ++i) {
                mtkfb_set_overlay_layer(info, &video_layer_context[i]);
            }
        }
...
////////////////////////////////////////////FM De-sense
	...
	case MTKFB_GET_INTERFACE_TYPE:
	{
        ...
        MTKFB_LOG("[MTKFB EM]MTKFB_GET_INTERFACE_TYPE is %ld\n", lcm_type);
    ...
    case MTKFB_GET_DEFAULT_UPDATESPEED:
	{
        ...
        MTKFB_LOG("[MTKFB EM]MTKFB_GET_DEFAULT_UPDATESPEED is %d\n", speed);
...
static int mtkfb_probe(struct device *dev)
{
	...
	if(p == NULL){
		lcd_fps = 6000;
		MTKFB_LOG("[FB driver]can not get fps from uboot\n");	
	}
	else{
		p += 4;
...
#endif
	MTKFB_LOG("[FB driver] fps = %d\n", lcd_fps);
	if(DISP_IsContextInited() == FALSE)
	{
		if(mtkfb_find_lcm_driver())
		{
			MTKFB_LOG("%s, we have found the lcm - %s\n", __func__, mtkfb_lcm_name);
			is_lcm_inited = TRUE;
		}
		else if(DISP_DetectDevice() != DISP_STATUS_OK)
		{
			MTKFB_LOG("[mtkfb] detect device fail, maybe caused by the two reasons below:\n");
			MTKFB_LOG("\t\t1.no lcm connected\n");
			MTKFB_LOG("\t\t2.we can't support this lcm\n");
		}
	}
	else
	{
...
/*                                              */
	...
	if(!overlay_buffer_head){
		MTKFB_LOG("[FB driver] vmalloc failed\n");
		r = -ENOMEM;
		goto cleanup;
	}
	...
	MTKFB_LOG("[FB Driver] fbdev->fb_pa_base = %x, fbdev->fb_va_base = %x\n", fbdev->fb_pa_base, (unsigned int)(fbdev->fb_va_base));
