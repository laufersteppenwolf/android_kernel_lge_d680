    ...
    *pu4SetInfoLen = sizeof(PARAM_CUSTOM_SW_CTRL_STRUC_T);
    ...
    /* 0xFFFE          reserved for FW */
        ...
        case 0x1001:
		  ...
		  tempSp = u4Data & BITS(0, 7);
		  if(0 == tempSp)
		  	tempSp = WMM_MAX_SP_LENGTH_ALL;
		  else if(2 == tempSp)
		  	tempSp = WMM_MAX_SP_LENGTH_2;
		  else if(4 == tempSp)
		  	tempSp = WMM_MAX_SP_LENGTH_4;
		  else if(6 == tempSp)
		  	tempSp = WMM_MAX_SP_LENGTH_6;
		  

		  prAdapter->u4MaxSpLen = tempSp;
		  prAdapter->rWifiVar.arBssInfo[u4Data>>16].rPmProfSetupInfo.ucUapsdSp= prAdapter->u4MaxSpLen;
	    }
        ...
        default:
            {
                printk( "wlanoidSwCtrlWrite 0x%x, 0x%x\n", prSwCtrlInfo->u4Id, prSwCtrlInfo->u4Data);
                rCmdSwCtrl.u4Id = prSwCtrlInfo->u4Id;
