...
#if defined(ENCRY_PARTITION_SUPPORT)
extern int LGE_FacReadWifiMacAddr(unsigned char *wifiMacAddr);
#endif
...
#else
        LGE_FacReadWifiMacAddr((unsigned char *)prRegInfo->aucMacAddr);
#endif
    ...
    )
{
    ...
    do {
        ...
        /* before start adapter, we need to open and load firmware */
        {
            ...
            //The default configuration for Sigma  Tool Test
	     prRegInfo->u4MaxSpLen = WMM_MAX_SP_LENGTH_6;
	     prRegInfo->u4UapsdAcBmp = PM_UAPSD_ALL;
...
/*
	...
	printk("wenwei, wlan remove enter\n");
    if(prGlueInfo->prAdapter->fgIsP2PRegistered) {
	
    	prGlueInfo->u4Flag |= GLUE_FLAG_RESET;
	printk("wenwei, wlan remove ...!!!\n");
        p2pNetUnregister(prGlueInfo, FALSE);
        p2pRemove(prGlueInfo);
	printk("wenwei, wlan remove here IsP2PRegistered\n");
    }
	printk("wenwei, wlan remove here!\n");
