...
** 
** 08 31 2012 cp.wu
** [ALPS00348418] [6577JB][WIFI][Performance]Search time is longer than reference Phone
** sync scan-fsm checking for network index instead of p2p-registered variable.
 *
...
/*                                                                                 */
#if 0
    if((UINT_8)prScanParam->eNetTypeIndex == 0) {/*NETWORK_TYPE_AIS_INDEX*/
       if( (UINT_8)prScanParam->eScanType == 1)/*SCAN_TYPE_ACTIVE_SCAN*/
       {
          rCmdScanReq.u2ChannelDwellTime = 60;// unit is in ms.
          rCmdScanReq.u2ChannelMinDwellTime = 60;//unit is in ms.
          printk("Active min scan time : %d\n",  rCmdScanReq.u2ChannelMinDwellTime);
       }
       else //Pasive scan
       {
          rCmdScanReq.u2ChannelDwellTime = 80;// unit is in ms.
          rCmdScanReq.u2ChannelMinDwellTime = 80;//unit is in ms.
          printk("Passive min scan time : %d\n",  rCmdScanReq.u2ChannelMinDwellTime);
       }
   }
#endif
/*                                                                                 */
