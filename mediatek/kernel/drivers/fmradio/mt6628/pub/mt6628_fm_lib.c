                           ...
                           fm_u16 *ScanTBLsize, fm_u16 scandir, fm_u16 space)
{
    ...
    num = (max_freq - min_freq) / 5; //Eg, (10800 - 8750) / 5 = 410
    seg = (num / 200) + ((num % 200) ? 1 : 0); //Eg, (410 / 200) + ((410 % 200) ? 1 : 0) = 2 + 1 = 3
    ...
    // do scan
    for (i = 0; i < seg; i++) {
        start_freq = min_freq + 1000 * i;
        end_freq = min_freq + 1000 * (i + 1) - 5;
        end_freq = (end_freq > max_freq) ? max_freq : end_freq;
        chl_cnt = 0;

        ret = mt6628_Scan(start_freq, end_freq, pFreq, scan_tbl, ScanTBLsize, scandir, space);
        if (ret < 0) {
            return ret;
        }
        ...
        for (ch_offset = 0; ch_offset < FM_SCANTBL_SIZE; ch_offset++) {
		    ...
		    for (step = 0; step < 16; step++) {
			    if (scan_tbl[ch_offset] & (1 << step)) {
                    tmp_val =  start_freq + (ch_offset * 16 + step) * 5;
                    WCN_DBG(FM_NTC | CHIP, "freq %d, end freq %d\n", tmp_val, end_freq);
                    if (tmp_val <= end_freq) {
                        chl_cnt++; 
                        // set reult bitmap
                        word_offset = (tmp_val - min_freq) / 5 / 16;
                        bit_offset = (tmp_val - min_freq) / 5 % 16;
