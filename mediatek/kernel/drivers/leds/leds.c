...
int setMaxbrightness(int max_level, int enable)
{
	...
	if (1 == enable)
	{
		limit_flag = 1;
		limit = max_level;
		LEDS_DEBUG("[LED] setMaxbrightness limit happen!!\n");
	}
	else
	{
		limit_flag = 0;
		limit = 255;
		LEDS_DEBUG("[LED] setMaxbrightness limit closed!!\n");
	}
 	mutex_unlock(&bl_level_limit_mutex);
	LEDS_DEBUG("[LED] setMaxbrightness limit_flag = %d, limit=%d\n",limit_flag, limit);
	return 0;
...
static int control_backlight_brightness(int level)
{
	...
	if (limit_flag == 1){
			...
			printk("mt65xx_leds_set_cust:value control! but value smaller than limit!\n");
		}
	printk("mt65xx_leds_set_cust:value control! do nothing!!!!\n");
