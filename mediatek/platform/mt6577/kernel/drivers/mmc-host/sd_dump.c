...
#define CMD_TIMEOUT        (500)        //500ms
#define DATA_TIMEOUT       (1000) //1000ms
#define BLK_LEN            (512)
...
static void msdc_config_clock(struct msdc_host *host, u32 hz)  /* no ddr */
{
    ...
    {    	
    	if (tmo-- <= 0){
    		printk("[%s]: waiting for CKSTB timeout\n", __func__);
    		break;
    	}
    	mdelay(1);
    }
...
static int msdc_cmd(struct msdc_host *host, u32 cmd, u32 raw, u32 arg, int rsptyp, u32*resp)
{
    ...
    if (intsts & MSDC_INT_CMDRDY) {
        ...
        printk("msdc cmd<%d> failed, intsts=0x%x \r\n", cmd, intsts);
        error = 1;  
        goto end; 	   	
    } 
...
static int msdc_pio_write(u32 *ptr, u32 size)
{
    ...
    while (left) {
        if ((left >= MSDC_FIFO_SZ) && (msdc_txfifocnt() == 0)) {
            ...
            printk("[%s]: wait for fifo clear timeout\n", __func__);
            msdc_clr_fifo();
            return 1;
        }else {
            udelay(1);
...
static int msdc_pio_read(u32 *ptr, u32 size)
{
    ...
    while (left) {
        if ((left >=  MSDC_FIFO_THD) && (msdc_rxfifocnt() >= MSDC_FIFO_THD)) {	
        	...
        	printk("[%s]: wait for fifo data timeout\n", __func__);
            return 1;
        }else {
            udelay(1);
        }
...
int msdc_write_sectors(unsigned char* dst, u32 blk_num, u32 nblks)
{
    ...
    if(!status){
        ...
        status = msdc_pio_write((u32 *)dst, totalLen); 
        ...
        if(!status){
            while( (intsts & MSDC_INT_XFER_COMPL) == 0 ){
                intsts = sdr_read32(MSDC_INT);	
                if(tmo-- == 0){
                   printk("[%s]: wait for MSDC_INT_XFER_COMPL timeout, intsts=0x%x, response=0x%x, address=0x%x\n", __func__, intsts, Response, data_address);
                   break;    
                }     
                mdelay(1);       
            }
            ...
            printk("[%s]: nblks=%d, response=0x%x, address=0x%x\n", __func__, nblks, Response, data_address);
            if (!msdc_cmd(host,  CMD13,  CMD13_RAW,  CMD13_ARG, RESP_R1, &Response)){
            	printk("[%s]: card status=0x%x, %s\n", __func__, Response, ((Response& OUT_OF_RANGE)?"out of range":" "));
            }
        }
    }
...
int msdc_read_sectors(unsigned char* dst, u32 blk_num, u32 nblks)
{
    ...
    if(!status){
        /* read the data out*/
        status = msdc_pio_read((u32 *)dst, totalLen); 	
...
static int SD_initialize(struct msdc_host *host)
{
    ...
    if (msdc_cmd(host, CMD8, CMD8_RAW, CMD8_ARG, RESP_R7,   &Response)){ 
        // SD v1.0 will not repsonse to CMD8, then clr HCS bit
        printk("SD v1.0, clr HCS bit\n");
        ACMD41_ARG = ACMD41_ARG_10;   	
    }else if (Response == CMD8_ARG) {
        // SD v2.0 will repsonse to CMD8, then set HCS bit
