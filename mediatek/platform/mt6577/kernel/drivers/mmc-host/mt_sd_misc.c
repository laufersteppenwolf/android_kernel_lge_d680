...
#ifdef AT_CMD_EMT
#define MSDC_MULTI_BUF_LEN  (128)
u32 msdc_multi_wbuf_misc[MSDC_MULTI_BUF_LEN];
u32 msdc_multi_rbuf_misc[MSDC_MULTI_BUF_LEN];	
u32 msdc_multi_backup_buf[MSDC_MULTI_BUF_LEN];	
...
{
    char cmdDataRequest[2048];                         //The AT command request
    ATOP_t opType;
    char cmdDataResponse[2048];                      //The AT command response
} cmd_data_type;

static int sd_card_valid(void)
{
	struct msdc_host *host_ctl, *host;
	struct scatterlist msdc_sg;
	struct mmc_data  msdc_data;
	struct mmc_command msdc_read_cmd;
	struct mmc_command msdc_stop_cmd;
	struct mmc_command msdc_write_cmd;
	struct mmc_request  msdc_mrq;
	...
	host_ctl = mtk_msdc_host[1]; //host1 for SD card.
	host = host_ctl;

	if(!host_ctl || !host_ctl->mmc || !host_ctl->mmc->card)
		return 0;

	printk("user want to check if there is a valid card in slot%d \n",host_ctl->id);

	if (mmc_card_present(host_ctl->mmc->card))
	{
		printk("[%s]: card present\n", __func__);
		mmc_claim_host(host_ctl->mmc);

		memset(&msdc_data, 0, sizeof(struct mmc_data));
		memset(&msdc_mrq, 0, sizeof(struct mmc_request));
		memset(&msdc_read_cmd, 0, sizeof(struct mmc_command));
		memset(&msdc_stop_cmd, 0, sizeof(struct mmc_command));
		memset(&msdc_write_cmd, 0, sizeof(struct mmc_command));
				
				
		//init read command
		msdc_read_cmd.opcode = MMC_READ_MULTIPLE_BLOCK;
		rPtr =(u8*)msdc_multi_rbuf_misc;
		for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN*4;forIndex++)
				*(rPtr + forIndex) = 0x0;		
		msdc_read_cmd.arg = address;	
		msdc_read_cmd.flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;

		//init write command
		msdc_write_cmd.opcode = MMC_WRITE_MULTIPLE_BLOCK;
		wPtr =(u8*)msdc_multi_wbuf_misc;
		for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN*4;forIndex++)
				*(wPtr + forIndex) = wData[forIndex%16];		
		msdc_write_cmd.arg = address;	
		msdc_write_cmd.flags = MMC_RSP_SPI_R1 | MMC_RSP_R1 | MMC_CMD_ADTC;

		if (!mmc_card_blockaddr(host_ctl->mmc->card)){
				msdc_read_cmd.arg <<= 9;
				msdc_write_cmd.arg <<= 9;
		}	
			
		//init stop command
		msdc_stop_cmd.opcode = MMC_STOP_TRANSMISSION;
		msdc_stop_cmd.arg = 0;
		msdc_stop_cmd.flags = MMC_RSP_SPI_R1B | MMC_RSP_R1B | MMC_CMD_AC;
		
		
		msdc_data.stop = &msdc_stop_cmd;
		msdc_data.blksz = 512;		
		msdc_data.sg = &msdc_sg;
		msdc_data.sg_len = 1;
		...
		msdc_data.flags = MMC_DATA_READ;
		msdc_data.blocks = sizeof(msdc_multi_rbuf_misc) / 512;
		msdc_data.bytes_xfered = 0;
		printk("[%s]: backup original data, blocks = %d\r\n", __func__, msdc_data.blocks);
		msdc_mrq.cmd = &msdc_read_cmd;
		msdc_mrq.data = &msdc_data;
		sg_init_one(&msdc_sg, msdc_multi_rbuf_misc, sizeof(msdc_multi_rbuf_misc));			
		mmc_set_data_timeout(&msdc_data, host_ctl->mmc->card);
		mmc_wait_for_req(host_ctl->mmc, &msdc_mrq);		
		rPtr =(u8*)msdc_multi_rbuf_misc;
		bPtr =(u8*)msdc_multi_backup_buf; 
		for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN*4;forIndex++)
				*(bPtr + forIndex) = *(rPtr + forIndex);					
		for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN*4;forIndex++)
				*(rPtr + forIndex) = 0x0;		
		for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN;forIndex++)
			printk("[%s]: original data: index:%d\t data:0x%x\r\n",__func__, forIndex, msdc_multi_backup_buf[forIndex]);		
		if(msdc_read_cmd.error || msdc_data.error){
			printk("[%s]: failed to back up the original data\r\n", __func__);
			goto fail;
		}
			
		//write new data		
		msdc_data.flags = MMC_DATA_WRITE;
		msdc_data.blocks = sizeof(msdc_multi_wbuf_misc) / 512;
		msdc_data.bytes_xfered = 0;
		printk("[%s]: write new data, blocks = %d\r\n", __func__, msdc_data.blocks);
		msdc_mrq.cmd = &msdc_write_cmd;
		msdc_mrq.data = &msdc_data;
		sg_init_one(&msdc_sg, msdc_multi_wbuf_misc, sizeof(msdc_multi_wbuf_misc));	
		mmc_set_data_timeout(&msdc_data, host_ctl->mmc->card);
		mmc_wait_for_req(host_ctl->mmc, &msdc_mrq);	
		if(msdc_write_cmd.error || msdc_data.error){
			printk("[%s]: failed to write new data\r\n", __func__);
			goto fail;
		}
		...
		msdc_data.blocks = sizeof(msdc_multi_rbuf_misc) / 512;
		msdc_data.bytes_xfered = 0;
		printk("[%s]: read and compare data, blocks = %d\r\n", __func__, msdc_data.blocks);
		msdc_mrq.cmd = &msdc_read_cmd;
		msdc_mrq.data = &msdc_data;
		sg_init_one(&msdc_sg, msdc_multi_rbuf_misc, sizeof(msdc_multi_rbuf_misc));			
		mmc_set_data_timeout(&msdc_data, host_ctl->mmc->card);
		mmc_wait_for_req(host_ctl->mmc, &msdc_mrq);		
		if(msdc_read_cmd.error || msdc_data.error){
			printk("[%s]: failed to read new data\r\n", __func__);
			goto fail;
		}
					
		for(forIndex=0;forIndex<MSDC_MULTI_BUF_LEN;forIndex++){					
			printk("index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]\t\n", forIndex, msdc_multi_wbuf_misc[forIndex], msdc_multi_rbuf_misc[forIndex]);
			if(msdc_multi_wbuf_misc[forIndex]!=msdc_multi_rbuf_misc[forIndex]){
		...
		//restore data
		msdc_write_cmd.error = 0;
		msdc_data.flags = MMC_DATA_WRITE;
		msdc_data.blocks = sizeof(msdc_multi_backup_buf) / 512;
		msdc_data.bytes_xfered = 0;
		printk("[%s]: restore data, blocks = %d\r\n", __func__, msdc_data.blocks);
		msdc_mrq.cmd = &msdc_write_cmd;
		msdc_mrq.data = &msdc_data;
		sg_init_one(&msdc_sg, msdc_multi_backup_buf, sizeof(msdc_multi_backup_buf));	
		mmc_set_data_timeout(&msdc_data, host_ctl->mmc->card);
		mmc_wait_for_req(host_ctl->mmc, &msdc_mrq);		
		if(msdc_write_cmd.error || msdc_data.error){
			printk("[%s]: failed to restore the original data after bit by bit compare\r\n", __func__);
		}
		mmc_release_host(host_ctl->mmc);

		//return result
		if(result == 0){
			printk("[%s]: data compare successed!! valid card\n", __func__);
			return 1; 
		}else{				
			printk("[%s]: data compare failed!! invalid card\n", __func__);			
			return 0;
		}
	}
	else{
		printk("[%s]: card is not present\r\n", __func__);
		return 0;
	}
fail:
	return 0;	
