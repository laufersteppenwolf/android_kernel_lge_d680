...
LGBmCableId LGBM_ReadUsbCableId( void )
{
	...
	for(i=0;i<5;i++)
	{
		...
		adc_avr+=adcValue[i];
		LGBM_DBG("ReadUsbCableId ( adcValue[%d] = %d ) adc_avr = %d\n", i, adcValue[i],adc_avr);
	    if( resVal < 0 )
	    {
...
//                                                                                                   
    ...
    {
        if((prevChgCurrent == LGBM_CC_I_SET) && (max8971_aicl_check()==1))
   		{
            LGBM_SetChargingCurrent(LGBM_CC_OFF);
			mdelay(20);
            LGBM_SetChargingCurrent(LGBM_CC_I_SET);	
		...
		else if((prevChgCurrent == LGBM_CC_USB_500) && (max8971_aicl_check()==1))
		{
            LGBM_SetChargingCurrent(LGBM_CC_OFF);
			mdelay(20);
            LGBM_SetChargingCurrent(LGBM_CC_USB_500);	
...
void BatThread_XGPTConfig(void)
{
    ...
    if( isFirstCalled == 1 )
    {
        ...
        config.u4CompareL = 6*512; // 10s : 512*64=32768
    }
    else
    {
