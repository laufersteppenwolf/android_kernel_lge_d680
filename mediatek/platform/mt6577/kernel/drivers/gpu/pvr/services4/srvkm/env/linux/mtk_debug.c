...
static void MTKPPAllocData(MTK_PROC_PRINT_DATA *data, int data_size, int max_line)
{
	...
	PVR_DPF((PVR_DBG_ERROR, "alloc + id=%p data:%p, line:%p", data, data->data, data->line));
...
static void MTKPPFreeData(MTK_PROC_PRINT_DATA *data)
{
	...
	PVR_DPF((PVR_DBG_ERROR, "free + id=%p data:%p, line:%p", data, data->data, data->line));
...
static IMG_VOID MTKPPInit(IMG_VOID)
{
	...
	{	
		{MTK_PP_R_SGXOSTimer,	MTK_PP_B_QUEUEBUFFER,	1024 * 1024 * 2,  1024 * 32},
		{MTK_PP_R_DEVMEM,		MTK_PP_B_RINGBUFFER,	1024 * 1024 * 2,  1024 * 32},
	};
    ...
    if (!g_MTKPP_proc)
    {
        PVR_DPF((PVR_DBG_ERROR, "%s: create /proc/gpulog create fail", __func__));
        goto err_out;
    }
