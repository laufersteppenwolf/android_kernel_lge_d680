...
int video_cpu_opp_mask[10] = {1,1,0,0,1,1,1,1,1,1};
#endif
...
static int mflexvideo_probe(struct platform_device *dev) 
{
    ...
    //Register Interrupt 
    if (request_irq(MT_VDEC_IRQ_ID , (irq_handler_t)mt6575_video_intr_dlr, 0, MFLEXVIDEO_DEVNAME, NULL) < 0)
    {
    ...
    if (request_irq(MT_VENC_IRQ_ID , (irq_handler_t)mt6575_video_intr_dlr2, 0, MFLEXVIDEO_DEVNAME, NULL) < 0)
    {
       MFV_LOGD("[MFV_DEBUG][ERROR] error to request MFlexVideo irq\n"); 
    }
    else
