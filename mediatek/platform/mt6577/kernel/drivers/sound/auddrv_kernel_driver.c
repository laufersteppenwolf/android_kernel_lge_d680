...
/*                                                                                        */
static int left_speaker;
static int right_speaker;
/*                                                                                        */
...
/*                                                                                         */		 
         if ( Reg_Data.offset == AFE_DAC_CON0 ) {
			xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv SET_AUDSYS_REG offset=%x, value=%x, mask=%x \n",Reg_Data.offset, Reg_Data.value, Reg_Data.mask);
		 }
/*                                                                                         */
         spin_lock_bh(&auddrv_lock);
...
// Set/Get AnalogAFE Register
	...
	/*                                                                                             */	 
	  //xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv SET_ANAAFE_REG Test offset=%x, value=%x, mask=%x \n",Reg_Data.offset,Reg_Data.value,Reg_Data.mask);
	  if(copy_from_user((void *)(&Reg_Data), (const void __user *)( arg), sizeof(Reg_Data))){
 
	     //xlog_printk(ANDROID_LOG_INFO, "Sound","AudDrv SET_ANAAFE_REG fail....\n");
	     AudDrv_Clk_Off();
            return -EFAULT;
         }
...
/*                                                                                        */
		if ( left_speaker == 0 && right_speaker == 1 ) {		// AAT Ring test -> 0 : ON, 1 : OFF
			Sound_Speaker_Turnon(2);							// Channel_Left = 2
	 	}
		else if ( left_speaker == 1 && right_speaker == 0 ) {	// AAT Camcorder test -> 0 : ON, 1 : OFF
			Sound_Speaker_Turnon(1);							// Channel_Right = 1
		}
		else {
			Sound_Speaker_Turnon(arg);							// Normal Mode
		}
/*                                                                                        */
...
/*                                                                                        */
static ssize_t L_speaker_set_show(struct kobject *kobj, struct kobj_attribute *attr,
                      char *buf)
{
    int var;
	...
	xlog_printk(ANDROID_LOG_ERROR, "Sound","L_speaker_set_show() left_speaker : %d right_speaker : %d\n",left_speaker, right_speaker);

    return sprintf(buf, "%d\n", var);
}

static ssize_t L_speaker_set_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int var;
	...
	xlog_printk(ANDROID_LOG_ERROR, "Sound","L_speaker_set_store() left_speaker : %d\n",left_speaker, right_speaker);

    return count;
}

static ssize_t R_speaker_set_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int var;

    var = right_speaker;

	xlog_printk(ANDROID_LOG_ERROR, "Sound","R_speaker_set_show() left_speaker : %d right_speaker : %d\n",left_speaker, right_speaker);

    return sprintf(buf, "%d\n", var);
}

static ssize_t R_speaker_set_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
    int var;
	...
	xlog_printk(ANDROID_LOG_ERROR, "Sound","R_speaker_set_store() right_speaker : %d\n",left_speaker, right_speaker);

    return count;
}

static struct kobj_attribute left_speaker_attribute =
        __ATTR(left_speaker, 0664, L_speaker_set_show, L_speaker_set_store);
static struct kobj_attribute right_speaker_attribute =
        __ATTR(right_speaker, 0664, R_speaker_set_show, R_speaker_set_store);

static struct attribute *attrs[] = {
    &left_speaker_attribute.attr,
    &right_speaker_attribute.attr,
...
static struct kobject *aat_speaker_set;
/*                                                                                        */
...
/*                                                                                        */
	...
	aat_speaker_set = kobject_create_and_add("kobject_speaker_set", kernel_kobj);
	if (!aat_speaker_set) {
		xlog_printk(ANDROID_LOG_INFO, "Sound","+kobject_create_and_add\n");
		return -ENOMEM;
	}

	retval = sysfs_create_group(aat_speaker_set, &attr_group);
	if (retval) {
		kobject_put(aat_speaker_set);
	}
	else {	
		xlog_printk(ANDROID_LOG_INFO, "Sound","+sysfs_create_group\n");
	}
/*                                                                                        */
