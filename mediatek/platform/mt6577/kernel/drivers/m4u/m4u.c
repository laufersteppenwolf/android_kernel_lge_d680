...
void m4u_profile_dump_reg(void)
{
    ...
    for (m4u_index=0; m4u_index<4; m4u_index++)
    {
        ...
        *(unsigned int*)(pM4UReg+0x800) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], 0x800);
        for (i=0x820; i<0x898; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
        for (i=0x8C0; i<0x8C8; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
        for (i=0x8F0; i<0x8F8; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
        for (i=0x900; i<0xA00; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
        for (i=0xA10; i<0xA18; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
        for (i=0xA20; i<0xA40; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
        for (i=0xB00; i<0xB20; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
        for (i=0xB40; i<0xB4C; i++)
            *(unsigned int*)(pM4UReg+i) = M4U_ReadReg32(gM4UBaseAddr[m4u_index], i);
    }
