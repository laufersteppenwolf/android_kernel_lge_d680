...
struct dram_overclock_driver {

    struct device_driver driver;
    const struct  platform_device_id *id_table;
};

static struct dram_overclock_driver dram_overclock_drv =
{
    .driver = {
        .name = "emi_clk_test",
...
int __init dram_overclock_init(void)
{
    ...
    ret = driver_register(&dram_overclock_drv.driver);
    if (ret) {
        printk("fail to create the dram_overclock driver\n");
        return ret;
    }
    ret = driver_create_file(&dram_overclock_drv.driver, &driver_attr_emi_clk_test);
    if (ret) {
