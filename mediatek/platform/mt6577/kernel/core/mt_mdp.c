...
void MT_MDP_DUMPREG(void);

/*=============================================================================
    Static Functions
  =============================================================================*/
static inline int MT_TimeStamp(unsigned long mdp_id, unsigned long a_u4Cmd , stTimeStamp * a_u4Param )
{
    ...
    if( a_u4Param != NULL )
    {
        ...
        {
            MDPK_ERROR("MT_TimeStamp copy from user failed!!\n");
            return -EFAULT;
        }
    ...
    u4BuffCnt = ioread32( MT_ROT_DMA_CFG_ADDR(pBase) );
    ...
    if(0 == a_u4Cmd)
    {
        ...
        do_div( sec, 1000 );                //sec = (usec)/ mod ignore
        usec = do_div( sec, 1000000);       //sec = (sec) / usec = mod (usec)
        
        u4RegVal = ioread32(MT_ROT_DMA_QUEUE_RSTA_ADDR(pBase));
...
static const char* MdpIoctlGetCmdStr( unsigned int    a_u4Command  )
{
    ...
    switch(a_u4Command)
    {
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_T_LOCKRESOURCE);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_T_UNLOCKRESOURCE);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_X_WAITIRQ);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_T_CLRIRQ);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_T_DUMPREG);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_G_TIMESTAMP);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_G_PMEMRANGE);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_T_SETZOOM);
    _IOCTLSTR_SWITCH_CASE_(MDPK_BITBLT_G_WAIT_REQUEST);
    _IOCTLSTR_SWITCH_CASE_(MDPK_BITBLT_T_INFORM_DONE);
    _IOCTLSTR_SWITCH_CASE_(MT_MDP_T_MVALOOPMEMTRACK);
...
static stZSDZoomSetting g_stZSDZoom = 
                    {
                        .SpinLock   = __SPIN_LOCK_UNLOCKED(g_stZSDZoom.SpinLock),
                        .u4Dirty    =   0,
                    };

static int MT_SetZSDZoom(stZSDZoomReg * a_u4Param)
{
    unsigned long flag;

    spin_lock_irqsave(&g_stZSDZoom.SpinLock , flag);
    g_stZSDZoom.u4Dirty= 0;
    spin_unlock_irqrestore(&g_stZSDZoom.SpinLock , flag);

    if(copy_from_user(&g_stZSDZoom.stReg , (struct stZSDZoomReg *)a_u4Param , sizeof(stZSDZoomReg)))
    {
...
#define MT_SYSRAM_PA 0xC2000000
...
static stZSDPreviewSetting g_stZSDPreview =
                    {
                        .SpinLock   = __SPIN_LOCK_UNLOCKED(g_stZSDPreview.SpinLock),
                        .u4Dirty    =   0,
                    };
...
static inline int _DescWriteWaitBusy( unsigned long reg_base )
{
    ...
    u4TimeOut = 0;
    while(!(ioread32(MT_ROT_DMA_QUEUE_WSTA_ADDR(reg_base)) & 0x1))
    {
        u4TimeOut += 1;
        //if(u4TimeOut  > 100000)
...
static inline int DescQueueRefill(unsigned long index)
{
    ...
    #define _CHECK_Q_BUSY_AND_RETURN_( _seg_name_ ) \
        if( _DescWriteWaitBusy( RGB_ROT0_BASE ) < 0 ){\
            ...
            return -1; }
    stZSDPreviewReg * pstReg = &g_stZSDPreview.stReg;

    /*Fill descriptor command*/
    /*[SEG1]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG1]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg1[index] , MT_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));

    /*[SEG4]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG4]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg4 , MT_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));

    /*[SEG5]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG5]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg5 , MT_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));

    /*[SEG6]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG6]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg6 , MT_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));

    /*[SEG7]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG7]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg7 , MT_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));
    /*[SEG8]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG8]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg8 , MT_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));

    /*[SEG9]*/
    _CHECK_Q_BUSY_AND_RETURN_("[SEG9]");
    mt65xx_reg_sync_writel(pstReg->u4RGB0Seg9 , MT_ROT_DMA_QUEUE_DATA_ADDR(RGB_ROT0_BASE));
...
static inline unsigned long UpdateZsdPreviewReg(void)
{
    ...
    mt65xx_reg_sync_writel(0x1, MT_RZ_LOCK_ADDR( CRZ_BASE ));
    mt65xx_reg_sync_writel(0x1, MT_ROT_DMA_LOCK_ADDR( RGB_ROT0_BASE ));

    reg_q_rsta  = ioread32(MT_ROT_DMA_QUEUE_RSTA_ADDR( RGB_ROT0_BASE ));
    reg_q_wsta = ioread32(MT_ROT_DMA_QUEUE_WSTA_ADDR( RGB_ROT0_BASE ));
    reg_q_base_pa = ioread32(MT_ROT_DMA_QUEUE_BASE_ADDR( RGB_ROT0_BASE ));
    reg_q_rd_base_pa = ioread32(MT_ROT_DMA_RD_BASE_ADDR( RGB_ROT0_BASE ));

    reg_q_depth = ioread32(MT_ROT_DMA_CFG_ADDR(RGB_ROT0_BASE));
    reg_q_depth = ((reg_q_depth & 0xf00) >> 8);
    reg_q_depth += 1;

    if(reg_q_base_pa < MT_SYSRAM_PA)
    {
        MDPK_ERROR("RGBROT0 sysram base address is weird 0x%08X\n" , (unsigned int)reg_q_base_pa);
        return ret;
    }
    reg_q_base_va = SYSRAM_BASE + (reg_q_base_pa - MT_SYSRAM_PA);
    ...
    if (pstReg->bStopAfterZSDDone == TRUE)
    {
        ...
        if (desc_remain_count >= 2)
        {
            ...
            {
            mt65xx_reg_sync_writel(0x1, MT_ROT_DMA_RPT_ADVANCE_ADDR(RGB_ROT0_BASE));
            }
    ...
    u4RegVal = ioread32(MT_RZ_CFG_ADDR(CRZ_BASE));
    u4RegVal &= (~(0x3FF << 16));
    u4RegVal |= (pstReg->u4CRZLBMAX << 16);
    mt65xx_reg_sync_writel(u4RegVal , MT_RZ_CFG_ADDR(CRZ_BASE));
    mt65xx_reg_sync_writel(pstReg->u4CRZSrcSZ , MT_RZ_SRCSZ_ADDR(CRZ_BASE));
    mt65xx_reg_sync_writel(pstReg->u4CRZTarSZ , MT_RZ_TARSZ_ADDR(CRZ_BASE));
    mt65xx_reg_sync_writel(pstReg->u4CRZHRatio , MT_RZ_HRATIO_ADDR(CRZ_BASE));
    mt65xx_reg_sync_writel(pstReg->u4CRZVRatio , MT_RZ_VRATIO_ADDR(CRZ_BASE));
    mt65xx_reg_sync_writel(pstReg->u4CRZCropLR , MT_RZ_CROPLR_ADDR(CRZ_BASE));
    mt65xx_reg_sync_writel(pstReg->u4CRZCropTB , MT_RZ_CROPTB_ADDR(CRZ_BASE));


    mt65xx_reg_sync_writel(0x0, MT_RZ_LOCK_ADDR( CRZ_BASE ));
    mt65xx_reg_sync_writel(0x0, MT_ROT_DMA_LOCK_ADDR( RGB_ROT0_BASE ));
...
static int MT_SetZSDPreview(stZSDPreviewReg * a_u4Param)
{
    unsigned long   flag;

    spin_lock_irqsave(&g_stZSDPreview.SpinLock , flag);
    g_stZSDPreview.u4Dirty= 0;
    spin_unlock_irqrestore(&g_stZSDPreview.SpinLock , flag);

    if(copy_from_user(&g_stZSDPreview.stReg , (struct stZSDPreviewReg *)a_u4Param , sizeof(stZSDPreviewReg)))
    {
    ...
    */
    ...
    {
        unsigned long y_cnt = (ioread32(MT_RZ_INFO0(CRZ_BASE))>>16) & 0x1FFF ;
        //unsigned long outcnt = ioread32(MT_RZ_INFO1(CRZ_BASE));
        unsigned long y_size = (ioread32(MT_RZ_SRCSZ_ADDR(CRZ_BASE))>>16) & 0xFFFF;

        if (y_cnt > y_size - 50)
        {
            spin_lock_irqsave(&g_stZSDPreview.SpinLock , flag);
            g_stZSDPreview.u4Dirty = 1;
            spin_unlock_irqrestore(&g_stZSDPreview.SpinLock , flag);
            g_stZSDPreview.stReg.u4DescUpdateZone = 0;
        ...
        {
            UpdateZsdPreviewReg();
            g_stZSDPreview.stReg.u4DescUpdateZone = 1;
    ...
    if(copy_to_user((struct stZSDPreviewReg *)a_u4Param, &g_stZSDPreview.stReg,
...
static inline void UpdateZsdZoom(void)
{
    ...
    stZSDZoomReg * pstReg = &g_stZSDZoom.stReg;

    if(g_stZSDZoom.u4Dirty)
    {
        /*Check VDO1 SYSRAM validity*/
        u4QueueBaseAddrRegVal = ioread32(MT_ROT_DMA_QUEUE_BASE_ADDR(VDO_ROT1_BASE));
        if(u4QueueBaseAddrRegVal < MT_SYSRAM_PA)
        {
            MDPK_ERROR("VDOROT1 sysram base address is weird 0x%08X\n" , (unsigned int)u4QueueBaseAddrRegVal);
            return;
        }

        mt65xx_reg_sync_writel(1 , MT_RZ_LOCK_ADDR(CRZ_BASE));
        mt65xx_reg_sync_writel(1 , MT_RZ_LOCK_ADDR(PRZ0_BASE));
        mt65xx_reg_sync_writel(1 , MT_ROT_DMA_LOCK_ADDR(VDO_ROT1_BASE));

        //Config VRZ first, since it is most time critical
/*
        u4RegVal = ioread32(MT_RZ_STA_ADDR(VRZ0_BASE));
        if(u4RegVal)
        {
...
*/
        ...
        mt65xx_reg_sync_writel(pstReg->u4VRZSrcSZ , MT_RZ_SRCSZ_ADDR(VRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4VRZTarSZ , MT_RZ_TARSZ_ADDR(VRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4VRZHRatio , MT_RZ_HRATIO_ADDR(VRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4VRZVRatio , MT_RZ_VRATIO_ADDR(VRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4VRZHRes , MT_VRZ_HRES_ADDR(VRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4VRZVRes , MT_VRZ_VRES_ADDR(VRZ0_BASE));
        ...
        //Config PRZ0
        u4RegVal = ioread32(MT_RZ_CFG_ADDR(PRZ0_BASE));
        u4RegVal &= (~(0x3FF << 16));
        u4RegVal |= (pstReg->u4PRZ0LBMAX << 16);
        mt65xx_reg_sync_writel(u4RegVal , MT_RZ_CFG_ADDR(PRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4PRZ0SrcSZ , MT_RZ_SRCSZ_ADDR(PRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4PRZ0TarSZ , MT_RZ_TARSZ_ADDR(PRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4PRZ0HRatio , MT_RZ_HRATIO_ADDR(PRZ0_BASE));
        mt65xx_reg_sync_writel(pstReg->u4PRZ0VRatio , MT_RZ_VRATIO_ADDR(PRZ0_BASE));


        //Config ROTDMA - modify sysram content
        u4QueueBaseAddrRegVal = ( SYSRAM_BASE + (u4QueueBaseAddrRegVal - MT_SYSRAM_PA) );//transfer to kernel virtual address
//        pu4DescriptBase = (unsigned long *)(u4RegVal);
        u4QDepth = ioread32(MT_ROT_DMA_CFG_ADDR(VDO_ROT1_BASE));
...
//Debug - E
#endif
        mt65xx_reg_sync_writel(0 , MT_ROT_DMA_LOCK_ADDR(VDO_ROT1_BASE));
        mt65xx_reg_sync_writel(0 , MT_RZ_LOCK_ADDR(PRZ0_BASE));
        mt65xx_reg_sync_writel(0 , MT_RZ_LOCK_ADDR(CRZ_BASE));
        g_stZSDZoom.u4Dirty = 0;
//Debug - S
//printk("ZSDZoom\n");
//Debug - E
...
static /*__tcmfunc*/ int MdpIsr_ROT( unsigned long mdp_id )
{
    ...
    desc_exec_counter = ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(pBase) );
    MDPK_INFO("ROT exe counter = %d\n", desc_exec_counter );

    
    //Debug:Show RPT 
    rpt = ioread32( MT_ROT_DMA_QUEUE_RSTA_ADDR(pBase) );
    ...
    reg_val = ioread32( MT_ROT_DMA_IRQ_FLAG_ADDR( pBase ) );

    //Clear interrupt
    mt65xx_reg_sync_writel( ( 0x33 & reg_val ), MT_ROT_DMA_IRQ_FLAG_CLR_ADDR(pBase) );
    ...
    if( reg_val & 0x1 )
    {
        ...
        if( g_mdp_irq_tbl.rot_exec_count[rot_index] != ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(pBase) ) )
        {
            g_mdp_irq_tbl.rot_exec_count[rot_index] = ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(pBase) );
        ...
        else
        {
            if(mdp_id == MID_RGB_ROT0)
            {
                drop_counter = ioread32( MT_ROT_DMA_DROPPED_FRAME_CNT_ADDR(pBase) );
...
static /*__tcmfunc*/ int MdpIsr_JPEGDMA( void )
{
    ...
    reg_val = ioread32( MT_JPEG_DMA_INTERRUPT_ADDR( pBase ) );
    
    /*Done*/
    if(reg_val & 0x1)
    {
        /*Clear*/
        mt65xx_reg_sync_writel( 0, MT_JPEG_DMA_INTERRUPT_ADDR( pBase ) );
...
static /*__tcmfunc*/ int MdpIsr_OVL( void )
{
    ...
    reg_val = ioread32( MT_OVL_DMA_IRQ_FLAG_ADDR( pBase ) );

    //Clear interrupt
    mt65xx_reg_sync_writel( ( 0x33 & reg_val ), MT_OVL_DMA_IRQ_FLAG_CLR_ADDR(pBase) );
    ...
    switch( mdp_id )
{
    ...
    #if 0
    {
        ...
        //Debug:Show Exec Counter
        desc_exec_counter = ioread32( MT_RDMA_EXEC_CNT_ADDR(pBase) );
        MDPK_INFO("RDMA exe counter = %d\n", desc_exec_counter );

        
        //Debug:Show RPT
        rpt = ioread32( MT_RDMA_QUEUE_RSTA_ADDR(pBase) );
    ...
    //Check error status as well
    reg_val = ioread32( MT_RDMA_IRQ_FLAG_ADDR(pBase) );
    //Clear interrupt 
    mt65xx_reg_sync_writel( (0x23 & reg_val), MT_RDMA_IRQ_FLAG_CLR_ADDR(pBase) );
...
static /*__tcmfunc*/ int MdpIsr_BRZ( void )
{
    ...
    reg_val = ioread32( MT_BRZ_INT_ADDR( pBase ) );
    ...
    {
        /*Clear*/
        //mt65xx_reg_sync_writel( ( reg_val | 0x03 ), MT_BRZ_INT_ADDR( pBase ) );
        mt65xx_reg_sync_writel( ( 0xF ), MT_BRZ_INT_ADDR( pBase ) );//Clear All, event no interrupt
    ...
    //Debug
    {
        ...
        if( b_en_regdump == 1 )
        {
            ...
            if( (current_ns - start_time) > 500000 ) /*500us*/
            {
...
static /*__tcmfunc*/ int MdpIsr_RESZ( unsigned long mdp_id )
{
    ...
    /*Read Clear*/
    reg_val = ioread32( MT_RZ_INT_ADDR(pBase) );
    ...
    if(reg_val & 0x2)
    {
        ...
        if( mdp_id == MID_CRZ )
        {
            ...
            if(1 == g_mdp_zoom_param.is_dirty && (1 < ABS(g_mdp_zoom_param.frame_counter, g_mdp_zoom_param.applied_frame)))
            {
                ...
                //Frame start
                mt65xx_reg_sync_writel(1 , MT_RZ_LOCK_ADDR(pBase));
                mt65xx_reg_sync_writel(g_mdp_zoom_param.CRZ_REG.reg_CFG , MT_RZ_CFG_ADDR(pBase));
                mt65xx_reg_sync_writel(g_mdp_zoom_param.CRZ_REG.reg_SRCSZ , MT_RZ_SRCSZ_ADDR(pBase));
                mt65xx_reg_sync_writel(g_mdp_zoom_param.CRZ_REG.reg_CROPLR , MT_RZ_CROPLR_ADDR(pBase));
                mt65xx_reg_sync_writel(g_mdp_zoom_param.CRZ_REG.reg_CROPTB , MT_RZ_CROPTB_ADDR(pBase));
                mt65xx_reg_sync_writel(g_mdp_zoom_param.CRZ_REG.reg_HRATIO , MT_RZ_HRATIO_ADDR(pBase));
                mt65xx_reg_sync_writel(g_mdp_zoom_param.CRZ_REG.reg_VRATIO , MT_RZ_VRATIO_ADDR(pBase));
                mt65xx_reg_sync_writel(0 , MT_RZ_LOCK_ADDR(pBase) );
    ...
    if( mdp_id == MID_CRZ )
    {
        ...
        /*Read CRZ_STA*/
        crz_sta_val = ioread32( MT_RZ_STA_ADDR(pBase) );
        ...
        /*Clear*/
        mt65xx_reg_sync_writel( (crz_sta_val&0xF8) , MT_RZ_STA_ADDR(pBase));
...
static /*__tcmfunc*/ int MdpIsr_VRZ( unsigned long mdp_id )
{
    ...
    /*Read Clear*/
    reg_val = ioread32( MT_VRZ_INT_ADDR(pBase) );
...
static /*__tcmfunc*/ irqreturn_t MdpIsr_Main(int irq, void *dev_id)
{
    ...
    switch( irq )
    {
        case MT_CRZ_IRQ_ID://CRZ
            MdpIsr_RESZ( MID_CRZ );
            break;

        case MT_ROT_DMA_IRQ_ID://RGB0/RGB1/VDO0/VDO1
        ...
        case MT_DISP_MDP_IRQ_ID://RDMA1/VRZ1/RGB2
...
static int MdpIsr_Register( const char *p_Name )
{
    ...
    _REQUEST_IRQ(MT_CRZ_IRQ_ID);
    _REQUEST_IRQ(MT_ROT_DMA_IRQ_ID);
    _REQUEST_IRQ(MT_OVL_JPG_DMA_IRQ_ID);
    _REQUEST_IRQ(MT_R_DMA0_IRQ_ID);
    _REQUEST_IRQ(MT_RESZ_IRQ_ID);
    _REQUEST_IRQ(MT_DISP_MDP_IRQ_ID);
...
static int MT_TryLockResource(unsigned long request_resource, int b_timeshare , stLockResParam*  p_stParam )
{
...
static int PowerManagement_RestInt( unsigned long mdp_id )
{
    ...
    switch ( mdp_id )
    {
        ...
        case MID_BRZ:
            ...
            //1.Disable INT
            reg_val = ioread32( MT_BRZ_INT_ADDR( pBase ) );
            mt65xx_reg_sync_writel( 0x0, MT_BRZ_INT_ADDR( pBase ) );//write 0x0 also clear INT
            //2.Clear INT
            //(Already clear)
            break;
        case MID_CRZ:
            pBase = CRZ_BASE;
            //1.Disable INT
            reg_val = ioread32( MT_RZ_CFG_ADDR( pBase ) );
            reg_val = ( reg_val & ~(0x3 << 10) );
            mt65xx_reg_sync_writel( reg_val, MT_RZ_CFG_ADDR( pBase ) );
            //2.Clear INT
            reg_val = ioread32( MT_RZ_INT_ADDR(pBase) );/*Read Clear*/
            break;
        case MID_PRZ0:
            pBase = PRZ0_BASE;
            //1.Disable INT
        ...
        case MID_VRZ0:
            pBase = VRZ0_BASE;
            //1.Disable INT
            reg_val = ioread32( MT_VRZ_CFG_ADDR( pBase ) );
            reg_val = ( reg_val & ~(0x1 << 10) );
            mt65xx_reg_sync_writel( reg_val, MT_VRZ_CFG_ADDR( pBase ) );
            //2.Clear INT
            /*Read Clear*/
            reg_val = ioread32( MT_VRZ_INT_ADDR(pBase) );
            break;
        ...
        case MID_R_DMA0:
            ...
            mt65xx_reg_sync_writel( reg_val, MT_RDMA_IRQ_FLAG_ADDR( pBase ) );
            //2.Clear INT
            reg_val = ioread32( MT_RDMA_IRQ_FLAG_ADDR(pBase) );
            mt65xx_reg_sync_writel( (0x23 & reg_val), MT_RDMA_IRQ_FLAG_CLR_ADDR(pBase) );
            break;
        ...
        case MID_OVL:
            ...
            mt65xx_reg_sync_writel( reg_val, MT_OVL_DMA_IRQ_FLAG_ADDR( pBase ) );
            //2.Clear INT
            reg_val = ioread32( MT_OVL_DMA_IRQ_FLAG_ADDR(pBase) );
            mt65xx_reg_sync_writel( (0x33 & reg_val), MT_OVL_DMA_IRQ_FLAG_ADDR(pBase) );
            break;
        /*        
        case MID_IPP:    
            break;
        ...
        */
        case MID_JPEG_DMA:
            pBase = JPG_DMA_BASE;
            //1.Disable INT
            reg_val = ioread32( MT_JPEG_DMA_CON_ADDR( pBase ) );
            reg_val = ( reg_val & ~(0x1) );
            mt65xx_reg_sync_writel( reg_val, MT_JPEG_DMA_CON_ADDR( pBase ) );
            //2.Clear INT
            mt65xx_reg_sync_writel( 0, MT_JPEG_DMA_INTERRUPT_ADDR( pBase ) );/*Clear*/
            break;
        case MID_RGB_ROT0:
            pBase = RGB_ROT0_BASE;
            //1.Disable INT
            reg_val = ioread32( MT_ROT_DMA_IRQ_FLAG_ADDR( pBase ) );
            reg_val = ( reg_val & ~(0x33 << 16) );
            mt65xx_reg_sync_writel( reg_val, MT_ROT_DMA_IRQ_FLAG_ADDR( pBase ) );
            //2.Clear INT
            reg_val = ioread32( MT_ROT_DMA_IRQ_FLAG_ADDR( pBase ) );
            mt65xx_reg_sync_writel( ( 0x33 & reg_val ), MT_ROT_DMA_IRQ_FLAG_CLR_ADDR(pBase) );
            break;
...
static int MT_UnLockResource(unsigned long a_u4ResTbl , stMDPOpLog * a_pstLog)
{
    unsigned long   irq_status;
...
static int MT_LockResource(unsigned long a_pParam , stMDPOpLog * a_pstLog)
{
    ...
    if( g_mdp_resource_tbl.resource_in_use_table & stParam.u4LockResTable ) 
    {
        ...
        ret_of_wait_timeout = wait_event_interruptible_timeout( 
                                    ...
                                    MT_TryLockResource(stParam.u4LockResTable, stParam.u4IsTimeShared , &stParam ) , //If success,realy update resource_in_use_table  
        ...
        {
            MT_UnLockResource( stParam.u4LockResTable, a_pstLog );
...
static int MT_WAIT_IRQ(unsigned long a_pParam)
{
    int             ret = 0;
...
static int MT_CLR_IRQ(unsigned long a_u4IRQFlag)
{
    unsigned long   flags;
    ...
    if( irq_flag & MID_RGB_ROT0 ) {
        g_mdp_irq_tbl.rot_exec_count[0] = ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(RGB_ROT0_BASE) );
    }
    if( irq_flag & MID_RGB_ROT1 ) {
        g_mdp_irq_tbl.rot_exec_count[1] = ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(RGB_ROT1_BASE) );
    }
    if( irq_flag & MID_RGB_ROT2 ) {
        g_mdp_irq_tbl.rot_exec_count[2] = ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(RGB_ROT2_BASE) );
    }
    if( irq_flag & MID_VDO_ROT0 ) {
        g_mdp_irq_tbl.rot_exec_count[3] = ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(VDO_ROT0_BASE) );
    }
    if( irq_flag & MID_VDO_ROT1 ) {
        g_mdp_irq_tbl.rot_exec_count[4] = ioread32( MT_ROT_DMA_EXEC_CNT_ADDR(VDO_ROT1_BASE) );
    }
...
static inline int MT_CheckPmemRange(stPMEMRange * a_u4Param)
{
    stPMEMRange input;
    ...
    pud = pud_offset(pgd, input.u4StartAddr);
    ...
    pmd = pmd_offset(pud, input.u4StartAddr);
...
static int MT_SetZoom( stZoomSetting *p_param )
{
...
static int MT_MvaLoopMemTrack( MDPIOCTL_MvaLoopMemTrack *p_param)
{
    unsigned long flag;
    ...
    {
        MDPK_ERROR("MDP MT_MvaLoopMemTrack: copy from user failed!!\n");
        return -EFAULT;
    }    
    ...
    if( copy_to_user( (struct MDPIOCTL_MvaLoopMemTrack *)p_param, &param, sizeof(MDPIOCTL_MvaLoopMemTrack) )) {
        MDPK_ERROR("MDP MT_MvaLoopMemTrack: copy to user failed!!\n");
        return -EFAULT;
    }
...
/*=============================================================================
    Member function for File operation
          ...
          static int MT_MDP_Open(struct inode * a_pstInode, struct file * a_pstFile)
          static int MT_MDP_Release(struct inode * a_pstInode, struct file * a_pstFile)
          static int MT_MDP_Flush(struct file * a_pstFile , fl_owner_t a_id)
          static int MT_MDP_Ioctl(struct inode * a_pstInode,
                            struct file * a_pstFile,
                            unsigned int a_u4Command,
                            unsigned long a_u4Param)
          static int MT_MDP_mmap(struct file * a_pstFile, struct vm_area_struct * a_pstVMArea)
  =============================================================================*/
...
static int MT_MDP_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if(pstLog->u4OwnResource)
    {
        ...
        MT_UnLockResource(pstLog->u4OwnResource , pstLog);
    }
                        ...
                        unsigned long   a_u4Param )
{
    ...
    if( ( pstLog->tgid != current->tgid ) 
        && ( a_u4Command != MT_MDP_T_LOCKRESOURCE) 
        //&& ( a_u4Command != MT_MDP_X_WAITIRQ)          //Deque will invoke in another thread
        && ( a_u4Command != MT_MDP_T_DUMPREG)            //Utility
        && ( a_u4Command != MT_MDP_G_PMEMRANGE)          //Utility
        ...
        && ( a_u4Command != MT_MDP_T_MVALOOPMEMTRACK )   //Register/unregister MVA before lock resource
    ...
    switch(a_u4Command)
    {
        case MT_MDP_T_LOCKRESOURCE :
            if( ( pstLog->tgid == 0 ) || ( pstLog->tgid == current->tgid) )
            {
                i4RetVal = MT_LockResource(a_u4Param , pstLog);
            } else
            {
                i4RetVal = -1;
        ...
        case MT_MDP_T_UNLOCKRESOURCE :
            i4RetVal = MT_UnLockResource(a_u4Param , pstLog);
            break;

        case MT_MDP_X_WAITIRQ :
            i4RetVal = MT_WAIT_IRQ(a_u4Param);
            break;

        case MT_MDP_T_CLRIRQ :
            MT_CLR_IRQ(a_u4Param);
            break;

        case MT_MDP_T_DUMPREG :
            MT_MDP_DUMPREG();
            break;

        case MT_MDP_G_TIMESTAMP :
            i4RetVal = MT_TimeStamp( (unsigned long)NULL, 1 , (stTimeStamp *)a_u4Param);
            break;
        
        case MT_MDP_G_PMEMRANGE :
            i4RetVal = MT_CheckPmemRange((stPMEMRange *)a_u4Param);
            break;

        case MT_MDP_T_SETZOOM:
            i4RetVal = MT_SetZoom((stZoomSetting *)a_u4Param);
            break;

        case MT_MDP_T_SETZSDZOOM:
            i4RetVal = MT_SetZSDZoom((stZSDZoomReg *)a_u4Param);
            break;

// last preview frame for zsd
        case MT_MDP_T_SETZSDPREVIEWFRAME:
            i4RetVal = MT_SetZSDPreview((stZSDPreviewReg *)a_u4Param);
            break;
        ...
        case MT_MDP_T_MVALOOPMEMTRACK:
            i4RetVal = MT_MvaLoopMemTrack((MDPIOCTL_MvaLoopMemTrack *)a_u4Param);
            break;
        ...
        default :
            MDPK_ERROR(" MDP ioctl : No such command!!\n");
            i4RetVal = -EINVAL;
        break;
    }
...
static long MT_MDP_Unlocked_Ioctl(struct file * a_pstFile,
                                      unsigned int a_u4Command,
                                      unsigned long a_u4Param )
{
    long            retval = 0;
...
/*=============================================================================
  ...
  =============================================================================*/
static dev_t                        g_mdp_device_number = MKDEV(MT_MDP_DEV_MAJOR_NUMBER,0);/*No need to set initial major #, because we use dynamic major number allocate*/
static struct cdev*                 g_mdp_char_device = NULL;
static const struct file_operations g_mdp_file_operations = 
                                    {
                                        .owner      = THIS_MODULE,
                                        .open       = MT_MDP_Open,
                                        .release    = MT_MDP_Release,
                                        .flush      = MT_MDP_Flush,
                                        
                                        #if defined( MDPK_FLAG_SUPPORT_UNLOCKED_IOCTL )
                                        .unlocked_ioctl = MT_MDP_Unlocked_Ioctl,
                                        #else
                                        .ioctl          = MT_MDP_Ioctl,
                                        #endif
...
static inline int RegisterMDPCharDrv(void)
{
    MDPK_SHOWFUNCTION( 1 );

    /*1. Kernel dynamic allocate major number*/
    if( alloc_chrdev_region(&g_mdp_device_number, 0, 1,"mt-mdp") ) /* FIXME. mt-MDP sync to MHAL and init.rc */
    {
        MDPK_ERROR("Allocate device # failed!\n");
        return -EAGAIN;
    }
    MDPK_INFO("device major:minor number = %d:%d\n\r", MAJOR(g_mdp_device_number),MINOR(g_mdp_device_number));

    /*2. Allocate a char devices*/
    g_mdp_char_device = cdev_alloc();

    if(NULL == g_mdp_char_device)
    {
        unregister_chrdev_region(g_mdp_device_number, 1);
        MDPK_ERROR("Allocate char device failed\n");
        return -ENOMEM;
    }

    /*3. Fill in char devices with file operations*/
    cdev_init(g_mdp_char_device, &g_mdp_file_operations);
    g_mdp_char_device->owner = THIS_MODULE;

    

    /*4. Tell the kernel this char device*/
    if(cdev_add(g_mdp_char_device, g_mdp_device_number, 1))
    {
        unregister_chrdev_region(g_mdp_device_number, 1);
...
static inline void UnregisterMDPCharDrv(void)
{
    MDPK_SHOWFUNCTION( 1 );
    
    /*Release char device*/
    cdev_del(g_mdp_char_device);
...
/*=============================================================================
    Member function for platform_driver
          ...
          static int MT_MDP_probe(struct platform_device *pdev);
          static int MT_MDP_remove(struct platform_device *pdev);
          static int MT_MDP_suspend(struct platform_device *pdev, pm_message_t mesg);
          static int MT_MDP_resume(struct platform_device *pdev);
          
  =============================================================================*/
static struct class *g_p_mdp_class = NULL;
...
static int MT_MDP_probe(struct platform_device *pdev)
{
    ...
    g_p_mdp_class = class_create(THIS_MODULE, "idpdrv");
    if (IS_ERR(g_p_mdp_class)) 
    {
        int ret = PTR_ERR(g_p_mdp_class);
        MDPK_ERROR("Unable to create class, err = %d\n", ret);
        return ret;            
    }    
    mdp_device = device_create(g_p_mdp_class, NULL, g_mdp_device_number, NULL, "mt-mdp");
...
static int MT_MDP_remove(struct platform_device *pdev)
{
    ...
    UnregisterMDPCharDrv();


    device_destroy(g_p_mdp_class, g_mdp_device_number);

    class_destroy(g_p_mdp_class);
...
static struct platform_driver g_mdp_platform_driver = 
{
    .probe		= MT_MDP_probe,
    .remove     = MT_MDP_remove,
    .suspend	= MT_MDP_suspend,
    .resume	    = MT_MDP_resume,
    .driver		= 
    {
        .name	= "mt-mdp",  /* FIXME. Sync to MHAL and init.rc */  /*Is this the name of device file under /dev/ ? */
        .owner	= THIS_MODULE,
    }
};
/*=============================================================================
...
void MT_MDP_DUMPREG(void)
{
    ...
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "MMSYS1", MMSYS1_CONFIG_BASE, MT_MMSYS1_REG_RANGE );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "RDMA0", R_DMA0_BASE, MT_RDMA0_REG_RANGE  );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "RDMA1", R_DMA1_BASE, MT_RDMA1_REG_RANGE  );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "BRZ", BRZ_BASE, MT_BRZ_REG_RANGE      );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "CRZ", CRZ_BASE, MT_CRZ_REG_RANGE          );
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "PRZ0", PRZ0_BASE, MT_PRZ0_REG_RANGE          );
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "PRZ1", PRZ1_BASE, MT_PRZ1_REG_RANGE          );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "VRZ0", VRZ0_BASE, MT_VRZ0_REG_RANGE          );
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "VRZ1", VRZ1_BASE, MT_VRZ1_REG_RANGE          );
    //------
    //MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "IPP", IMGPROC_BASE, MT_IPP_REG_RANGE  );
    //------
    //MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "OVL", OVL_DMA_BASE, MT_OVL_DMA_REG_RANGE   );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "RGB0", RGB_ROT0_BASE, MT_RGB_ROT0_REG_RANGE );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "RGB1", RGB_ROT1_BASE, MT_RGB_ROT1_REG_RANGE );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "RGB2", RGB_ROT2_BASE, MT_RGB_ROT2_REG_RANGE );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "VDO0", VDO_ROT0_BASE, MT_VDO_ROT0_REG_RANGE );
    //------    
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "VDO1", VDO_ROT1_BASE, MT_VDO_ROT1_REG_RANGE );
    //------
    MdpRegDump( 1 , NULL, &off_current, off_start, off_end, &over, "JPGDMA", JPG_DMA_BASE, MT_JPEG_DMA_REG_RANGE  );
                            ...
                            int count,  int *eof, void *data )
{
    ...
    *start  = page;
    ...
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "MMSYS1", MMSYS1_CONFIG_BASE, MT_MMSYS1_REG_RANGE );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "RDMA0", R_DMA0_BASE, MT_RDMA0_REG_RANGE  );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "RDMA1", R_DMA1_BASE, MT_RDMA1_REG_RANGE  );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "BRZ", BRZ_BASE, MT_BRZ_REG_RANGE      );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "CRZ", CRZ_BASE, MT_CRZ_REG_RANGE          );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "PRZ0", PRZ0_BASE, MT_PRZ0_REG_RANGE          );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "PRZ1", PRZ1_BASE, MT_PRZ1_REG_RANGE          );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "VRZ0", VRZ0_BASE, MT_VRZ0_REG_RANGE          );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "VRZ1", VRZ1_BASE, MT_VRZ1_REG_RANGE          );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "IPP", IMGPROC_BASE, MT_IPP_REG_RANGE  );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "OVL", OVL_DMA_BASE, MT_OVL_DMA_REG_RANGE   );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "RGB0", RGB_ROT0_BASE, MT_RGB_ROT0_REG_RANGE );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "RGB1", RGB_ROT1_BASE, MT_RGB_ROT1_REG_RANGE );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "RGB2", RGB_ROT2_BASE, MT_RGB_ROT2_REG_RANGE );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "VDO0", VDO_ROT0_BASE, MT_VDO_ROT0_REG_RANGE );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------    
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "VDO1", VDO_ROT1_BASE, MT_VDO_ROT1_REG_RANGE );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
    //------
    byte_out += MdpRegDump( 0 , page+byte_out, &off_current, off_start, off_end, &over, "JPGDMA", JPG_DMA_BASE, MT_JPEG_DMA_REG_RANGE  );
    printk("byte_out=%d  off_current=%lu over=%d\n", byte_out,off_current, over);
    if( over ) goto end;
...
static int __init MT_MDP_Init(void)
{
    MDPK_SHOWFUNCTION( 1 );

    if(platform_driver_register(&g_mdp_platform_driver))
    {
        MDPK_ERROR("failed to register  MDP driver\n");
        return -ENODEV;
    }
...
static void __exit MT_MDP_Exit(void)
{
    MDPK_SHOWFUNCTION( 1 );
    platform_driver_unregister(&g_mdp_platform_driver);
}
...
module_init(MT_MDP_Init);
module_exit(MT_MDP_Exit);
MODULE_DESCRIPTION("MDP driver");
