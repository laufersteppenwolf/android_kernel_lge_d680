...
void arch_idle(void)
{
	...
	if(arch_idle_count == ARCH_IDLE_COUNT_MAX)
	{	    
		mode = *((volatile UINT32 *)0xF1019100) & (0x1 << 28);	
		if(mode)
		{
		    if(*((volatile UINT32 *)ARMPLL_CON0) == 0x48A0)
		    {
			  /* do nothing */
		    }
		    else
		    {
		        printk(KERN_ERR "0xF0007100 should be 0x48A0, but now it is 0x%x\n", *((volatile UINT32 *)ARMPLL_CON0));
		    }
		}
		else
		{
		    if(*((volatile UINT32 *)ARMPLL_CON0) == 0x4CA0)
		    {  
		        /* do nothing */
		    }
		    else
...
void enable_low_power_settings(void)
{
    ...
    mt_irq_unmask_for_sleep(MT_KP_IRQ_ID);
}
...
static int primary_cpu_enter_wfi(void *data)
{
    ...
    mt_irq_mask_all(&cpu_irq_mask[cpu]);
    mt_irq_unmask_for_sleep(MT6577_KP_IRQ_ID);
    ...
    spin_lock(&factory_lock);
    mt_irq_mask_restore(&cpu_irq_mask[cpu]);
    spin_unlock(&factory_lock);
...
void mt6577_dcm_init(void)
{
	...
	arm_pm_idle = arch_idle;//3.4 change
