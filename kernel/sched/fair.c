...
{
	if (!atomic_cmpxchg(&cfs_rq->on_list, 0, 1)) {
		/*
		 ...
		 */
		if (cfs_rq->tg->parent &&
		    atomic_read(&(cfs_rq->tg->parent->cfs_rq[cpu_of(rq_of(cfs_rq))]->on_list))) {
...
{
	if (atomic_read(&cfs_rq->on_list) &&
	    (cfs_rq->leaf_cfs_rq_list.prev == LIST_POISON2)) {
		printk(KERN_ERR "%s: double free, cpu %d curr %p nr_running "
				"%lu load_avg %llu\n",
				__func__, rq_of(cfs_rq)->cpu, cfs_rq->curr,
				cfs_rq->nr_running, cfs_rq->load_avg);

		if (cfs_rq->curr && entity_is_task(cfs_rq->curr)) {
			struct task_struct *p = task_of(cfs_rq->curr);
			printk(KERN_ERR " curr is a task %d:%s\n",
					task_pid_nr(p), p->comm);
		}
	}

	if (atomic_cmpxchg(&cfs_rq->on_list, 1, 0)) {
...
void unregister_fair_sched_group(struct task_group *tg, int cpu)
{
	...
	*/
	if (!atomic_read(&(tg->cfs_rq[cpu]->on_list)))
		return;
