	...
	spin_lock_irqsave(&cdev->lock, flags);
		...
		next_state = dev->connected ? USB_CONNECTED : USB_DISCONNECTED;
		 if(!is_hwconnected){
			...
			next_state =USB_HWDISCONNECTED;
		}
	/*                                                                              */	 
	} else if(!is_hwconnected && (last_uevent != USB_HWDISCONNECTED)) {
	/*                                                            */
		uevent_envp = hwdisconnected;
		next_state =USB_HWDISCONNECTED;
		xlog_printk(ANDROID_LOG_VERBOSE, USB_LOG,"%s: last_uevent=%d \n", __func__, last_uevent);
	}
	/*                                    */
#else
	if(upmu_is_chr_det()) {
