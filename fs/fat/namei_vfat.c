				  ...
				  struct nameidata *nd)
{
	...
	if (err) {
		xlog_printk(ANDROID_LOG_INFO, FAT_TAG, 
			"%s() : vfat_find(%s) failed, err=%d, current process is \"(PID=%d)%s\"\n", 
			__func__, 
			(dentry->d_name).name,
			err,
			current->pid,
	...
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		xlog_printk(ANDROID_LOG_INFO, FAT_TAG, 
			"%s() : fat_build_inode() failed, err=%d, current process is \"(PID=%d)%s\"\n", 
			__func__, 
			err,
			current->pid,
			current->comm
...
error:
	xlog_printk(ANDROID_LOG_INFO, FAT_TAG, 
			"%s() failed, err=%d, current process is \"(PID=%d)%s\"\n", 
			__func__, 
			err,
			current->pid,
			current->comm
